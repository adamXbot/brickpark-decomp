 ...
 *    measured.
 * ========================================================================= */
// WIP-FUNCTION: LEGOLAND 0x004284d0  (73.4%: 143/143 insns and 528/528 BYTES over the true body 0x004284d0..0x004286df, 38 strict mismatches, first divergence at index 60. NOTE audit.py cannot bound this function: the `jmp 0x428695` at 0x0042868f is an unconditional jump that no EARLIER branch crosses, so the walker stops there and reports 112i/449B -- ours is 112/112 and 449/449 in that window too -- and then flags ESCAPES because the real body continues to 0x004286df. It therefore cannot print [OK] however exact the reconstruction is. The residual: all four scale loops (indices 0-59) and the entire straight-piece double loop (indices 106-142) are exact index for index; everything left is register allocation in the curve loop's argument setup -- the original keeps the entry heading in eax and 3*b in ecx where this build uses ecx and edx, and the two `inc esi` land one slot later. Free `volatile` reads at all four load sites move it by at most 1 (38 -> 37), which by the recorded one-experiment test makes it a global web rank, not a reachable construct)
