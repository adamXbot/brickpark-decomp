...
## Seventh pass — phi and loop-carried webs, 2026-09-07

This pass tested the remaining back-end-specific gap rather than permuting the
cursor stores: whether C2 preserves a loop phi or a two-predecessor phi long
enough to put the byte constraint on a fresh post-copy web. Scratch is
`/tmp/svclear7_alloc.c`; the two real-body substitutions are generated by
`/tmp/svclear7_variants.py`. CLEAR remains **WIP at 177i/576B, 24 strict,
first difference 110**.

The scratch results establish the following:

- `tx = flag ? x : sq.x` after the 6196-byte copy folds completely once
  `sq.x = x` is visible, even when `&sq` escapes later. There is no phi and
  the byte store uses x's low register directly.
- A loop-carried `tx`, initialized and redefined from x around a byte store,
  also folds to x's web. C2 keeps x in `dl` across `rep movsd`; it does not
  create an uncoalesced fix-up copy. A loop-carried narrowed y value likewise
  stays in y's low register, and a later `sq.y` read forwards from y rather
  than reloading the local home.
- Putting the same block copy on opposite sides of a join does not make the
  intrinsic an edge-specific interference point: C2 hoists the common
  `rep movsd`, deletes the join and again forwards both bytes.
- A *real* non-identical join does create the late byte-class temp. With
  `if (flag) tx = x; else tx = *(volatile int*)&sq.x;`, scratch emits
  `mov ecx, edx` on the register edge, `mov ecx,[esp+8]` on the memory edge,
  and the byte store from `cl`. Thus C2 attaches the byte class to the phi
  result and inserts an edge fix-up; it does not require both incoming webs
  to be byte-capable. The cost is an unavoidable test, branch and alternate
  load, none of which exists in CLEAR's original group.
- Forming `unsigned char ny = (unsigned char)y` before `rep movsd` while
  keeping y live afterward does not split the web. `ny` coalesces with y and
  survives in the same low register; there is no narrowed spill or reload.

Only the real phi mechanism was ported. A y phi (register `by` versus volatile
`sq.y`) compiles to **180i/586B, scorer strict 116, first 19, rb 13, ob 8**;
an x phi paired with the y home reload gives **178i/582B, strict 109, first
19, rb 12, ob 6**. Both move `next` to ebp and break the already-exact load
and footprint blocks. The affinity edge from the byte-class phi therefore
propagates enough pressure to produce the ordinary two-byte-need colouring;
it cannot hide x's need or preserve y's need without retaining the branch.

No source change was promoted. The retained body is still the measured floor,
and loop-carried webs, rep-copy edge phis, early narrow temps, and genuine
non-identical phis are now bounded negatives. Final gates: `eventtick.c`
