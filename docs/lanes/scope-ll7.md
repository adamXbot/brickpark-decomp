...
| 0x00428860 | TrackShade_FillPoly | 254 | 69 | 254/254i, 765/771B, 175/254=68.9%, **0x70, exact through imul** | WIP |

**16 / 17 exact.** FillPoly is now size-exact (254i) with the original 0x70 frame; matchfull 175/254 = 68.9%. Residual is the zrow-load vs `shl eax,1` / nshade interleave after the product. `/W3` clean. `relocs.py` 0 MISMATCH on the 16 FUNCTION bodies.
...
- **TrackShade_FillPoly**: 0x70 frame, 254/254i, matchfull 175/254 = 68.9%. The `g_zb_polys++` ebx-vs-eax / hoisted-y residual closed by splitting `y` out of `ShadeSetup` and address-taking it with a late `y = *(int volatile*)&y + 1` (ZBuffer's `*(int volatile*)&y` on a struct field grew the frame to 0x74 and made `y` an imul temporary). `keys[*(int volatile*)&nkeys].y` plus `keys = *(SortKey* volatile*)&keys` before the outer loop put keys in edx (not ebx) so the sentinel is `mov [edx+ecx*8],eax`. Named `py = pitch * y; py <<= 1; crow = (char*)c + py` lands `imul eax,[ebp-4]` and `shl eax,1`. Volatile zrow *after* the crow store is the 68.9% attractor (exact prefix through insn 86); volatile zrow *before* the store hoists the load to insn 84 and slides the imul. Ordinary named `c`/`z` together hoist crow into edi before the sentinel. `py & 0` / `py * 0` dependencies on the zrow load were killed. Still ruled out: dummy `dead`; `&s.y` / `volatile int y` field (0x74); `int slot[8]` (0x74). Residual: zrow load vs `shl` / nshade-in-eax after the crow store.
