...
**7 of 8 exact.** One honest WIP (`FootprintClearanceTest`) at its FR02 floor.
...
- **FootprintClearanceTest**: `oy` loaded before `ox`; assign into `y` (and `x`) so y0 lives in the y-arg slot; outer latch as `if (yy >= end) goto success; goto loop;` so fail sits between the jmp and success (`jge` + `jmp`, not inverted `jl`). Residual: every 4-byte address-taken out-local (`void*`, `struct {void*}`, `void*[1]`, `char[4]`, function-level or block-scope) homes in the dead x-arg — no `push ecx`. An 8-byte `{elem, pad}` aggregate is `sub esp, 8` (too big). `int hole = 0` after the owner load is DCE'd. An inline FindElement helper that owns `elem` destroys the ebx/ebp ranking (46.6%). Size 83i/240B vs 85i/242B. §6B: frame-layout (`strict` is the missing slot plus displacement ripple); 4-byte fresh slot and dead-arg reuse are the only two homes VC6 will give this IR.
...
`relocs.py`: 0 MISMATCH, 1 UNRESOLVED (Unload's `"SPEECH BUBBLE"` literal — same as text.c's Load). `/W3` clean. `audit.py` PASS, seven `[OK]`.
