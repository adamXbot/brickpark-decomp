...
> **PORT-B6 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-B6)** — **ZERO
> TRAPS, AND THE FRONT END ANSWERS THE MOUSE.** `avifil32.c` (all 16 AVIFile
> entry points; `AVIFileOpenA` reports AVIERR_FILEOPEN, which is the path
> movie.c/advisor.c are written for — the advisor degrades to no animation, not
> a crash, because `RenderAdvisorIcon`'s draw is behind `if (g_vidanim)`),
> `msacm32.c` (a REAL PCM -> PCM16 converter, because data2.c runs every sample
> through it and drops what fails) and WINSPOOL's `EnumPrintersA` in gdi32.c:
