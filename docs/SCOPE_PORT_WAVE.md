...
> **PORT-A9 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A9)** — the two gates B11 §3 and M10 §1f asked for, and both found more: `gen/rawwords.md` is a DECLARATION-INDEPENDENT census (every emitted object, image-range values, A2's text vetoes, string-START corroboration) — 27 objects / 107 words that `raw pointer words: 0` could never see, gated against a baseline by the ctest `raw_words`; an unbounded array of a struct with pointer fields is now tiled into COMPLETE elements for the pointer scan (72 words re-pointed incl. g_power_table's 63 element names; address-keyed bytes proof 21,397/0/0/72 wasm, 85,495/0 native; without the text veto it re-pointed 55 words of the CREDITS into g_zbuf_pixels); `portable/tools/bvstruct_sweep.py` with an emcc-vs-clang selftest and 2 ctests — reading the sources as the PORTABLE build and sizing typedefs tree-wide turns M10's 8 silent sites (3 of them its own fixes) into 20 over 15 addresses, **13 new live ones** (boating school, restaurant start/stop sound, jungle cruise, media shop, mechanics hut, school cars); `legoland_headless --probe-audio` counts CreateSoundBuffer through a wrapped vtable — 4 where B11 measured 1, 24 with M11's bounds. wasm ctest 24/24, native 17/17. PORT-M11 collides in 2 files, measured in the notes §5. Notes `docs/lanes/scope-port-a9.md`
