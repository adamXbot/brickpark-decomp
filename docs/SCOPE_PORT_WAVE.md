...
> **PORT-A10 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A10)** — the dual-address gate: PORT-P3 §3.3 promoted to `portable/tools/addr_sweep.py` (two checks, a positive control for each, ctests `addr_sweep` + `addr_sweep_selftest` against `portable/tests/addr_collisions.txt`, a row that stops being reported prints FIXED and passes so it cannot race PORT-M15) — **18 names, not 17**: the extra one is the FUNCTION `Track_Update`, defined twice (0x004275d0 coaster.c, 0x00427b20 castleobj.c) and already mitigated by PORT-M7's `#define`, plus 5 addresses carrying names of incompatible declared size, all ruled on. gen_link stops choosing silently: `names with conflicting addresses: 18`, an attributed manifest table, a stderr warning and a `->`-marked block at the top of globals.c (`-DLL_FAIL_ON_ADDR_COLLISION` makes it a real `#error`). P3-4 is TWO bugs — `llLink`/`llPad` compared only the LLIDB element name, AND `ObjDef +0x04` is not a complete instance list (lesson 1: "Park Entrance" has an EMPTY chain and 120 cells on the map) — both fixed, with new `llFind`/`llClasses`/`llMapObjects`. P3-7 measured: a hidden tab runs full rate for THIRTY SECONDS then drops to exactly 0.50 fps; rAF is suspended, a Worker timer costs a round trip per yield, and a MessageChannel hop (0.003 ms, not a timer, no audio permission) holds **35.8 fps** — behind `?awake=1`, default ON for the testing page. Native 19/19, wasm 26/26. Notes `docs/lanes/scope-port-a10.md`**
