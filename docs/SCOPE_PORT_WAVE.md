...
> **PORT-B7 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-B7)** — THE NAME EDITOR WORKS: a click opens the NEW PROFILE popup and "adam" is typed into `g_temp_profile.name` and drawn with its cursor, with NO shim change needed (DIK map, press latch and scancode-vs-WM_CHAR all already right). The page gained a driver in game pixels (llMove/llClick/llKey/llType/llAlive/llPeek), `ll_dbg_addr` into the game's live globals, `legoland_browser_named` (-g2, so name_trap can name an indirect-call mismatch's callers), and a TRAP banner for the RuntimeError ASYNCIFY used to swallow. BLOCKER, a matching lane's one-liner: spritemisc.c:23-26 declares the sprite owner's vtable +0x08 `void` where sprite2.c/printlist.c/gpu.c declare the same IDirectDrawSurface `Release` slot `long`, so the first cached-text sprite to expire kills the module ~0.4 s after that click. With it corrected in a throwaway build the walk reaches the main menu, the tutorial-level list and the park-advert screen, all at 33 fps; THE PARK WAS NOT REACHED. Notes `docs/lanes/scope-port-b7.md`
