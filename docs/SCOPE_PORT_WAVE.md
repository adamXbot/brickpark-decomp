...
> **PORT-B11 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-B11)** — THE GAME HAS ITS OWN TYPE AND ITS OWN VOICE. PARK-4 closed: a TrueType rasteriser (`ll_ttf.c`, written here, not vendored) reads the shipped `Lego.TTF` and GDI's cell-height mapping makes the tutorial letter's longest line measure 383 px of its 460 — the only one of the four LOGFONTs that fits, and it is the one text.c's addresses name, so the font file and the shipped line lengths agree. `msacm32.c` gained a real MS ADPCM decoder (archived samples convertible 134/155 -> 155/155; speech 0 -> 1,266/1,266, byte-identical to an independent reference) and `ll_audio.c` puts Web Audio behind B4's silent IDirectSound: **the narration speaks** — 29.6 s of streamed speech, 426 of 440 chunks carrying waveform, 2 underruns, 33.9 fps, traps []. Native ctest 14/14, wasm 20/20. Open, both named and neither this lane's: sound EFFECTS never load because `extern FXEntry g_game_fx[];` has no bound, so 23 sample-name pointers stay RAW x86 VAs and `pointers.md`'s "raw pointer words: 0" is blind to it (122 raw words in 18 objects tree-wide, including 63 in `g_power_table`) — PORT-M, one line per declaration; and PARK-2 is settled (the surface model is innocent, proved both ends) and re-filed as `avifil32.c`'s advisor stub, a PORT-B sibling of PARK-4. Notes `docs/lanes/scope-port-b11.md`

> **PORT-M11 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-M11)** — closes the two classes PORT-M10 §1f and PORT-B11 §3 left open: the dangerous direction of the by-value-struct class (the DEFINITION takes the struct), and the unbounded-`extern T name[]` class that makes `gen_link.py`'s pointer scan skip a whole object. Notes `docs/lanes/scope-port-m11.md`
