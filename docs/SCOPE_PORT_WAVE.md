...
> **PORT-A7 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A7) — BOTH DOORS INTO THE PARK OPEN.** B8's B2 and B2a closed in the generator: pointer-ness came from the top-level declaration's `*` depth, so `extern LevelMarker g_low_markers[5]` contributed no pointer slots and its ten sprite-name MEMBERS kept the original binary's addresses (A2's resolver was right and was never asked). `cdecl.pointer_offsets` walks the laid-out type, keyed by ADDRESS; interior re-points 441 -> 693 (+252: g_fp_table 130, g_build_followups 46, g_level_markers 18, g_low_markers 10), 0 of 21,368 other cells differ. `raw pointer words: 0` is a manifest line and the ctest `pointer_words`; 12 left raw with a reason (all a `void*` over a number — a matching lane's list). In a tab: the tutorial tick draws, `llClick(577,419)` returns in 827 ms into the in-game screen (hash `0x561128cd`, 34 fps, 0 traps) and (577,300) in 824 ms. OPEN: the park does not RENDER yet, and a park toolbar click is a `call_indirect` type mismatch. Notes `docs/lanes/scope-port-a7.md`
