...
> **PORT-M3 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-M3)** — the typed-callback pass: every function-pointer TABLE and slot the game calls through, censused against the real wasm signature of every body stored in it, and the mismatches closed under `LEGOLAND_PORTABLE` arms. Notes `docs/lanes/scope-port-m3.md`
