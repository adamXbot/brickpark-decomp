...
> **PORT-P3 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-P3)** — PLAY tutorial lessons 4 and 5. Both load, render and run trap-free at ~34 fps; lesson 4 reaches objective 3 of 9, lesson 5 objective 1 of 9, and BOTH stop at the objective that asks for a worker. Two blockers, same class (one NAME, two ADDRESSES): P3-1 `scrolltick.c`'s `g_view_left/top/right/bottom` (0x004b95f4) lose the name to coaster3d.c's (0x008299ac), so ClampScrollToMap reads 0 slack and the map's north-east — the Mechanic's Hut — cannot be scrolled to (measured: the east limit is exactly `2*g_scroll_y - (view_w<<8)` on both maps); P3-2 `fpui2.c` declares `g_popup` at 0x007fdec0 where bighelp.c/popup.c say 0x007fdea4, so every PopUpInfoSetUp field is 0x1c low, `elem_shed`/`elem_hut` read 0 and NO gardener or mechanic can ever be hired. P3-3: 15 more colliding names swept. NB tutorial lesson 5 is the BOATING SCHOOL and the Wild West — there is no coaster or flume in any tutorial (they start at game level 2, ObjList7/GLTWO). Notes `docs/lanes/scope-port-p3.md`
