...
> **PORT-M13 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-M13)** — the player-reported "the game keeps asking me to link the Space Tower" is the GAME'S OWN RULE, correctly ported: LINK tests exactly ONE square, the ride's entrance offset added to its base (the square the entrance arrow points at), and wants a PathSquare there carrying flag 2, the entrance flood fill's reachable bit. Proved by poking the class's entrance offset in the live heap: a ride on a fully connected pad FAILS when that one square is grass and PASSES on one hand-laid square there. Eight hand-laid path shapes measured: every connected one satisfies, every dead end or corner-only touch does not; a built ride is normally linked already because RefreshObjList paves the footprint's one-cell clearance ring (which always covers the entrance) and bridges a ONE-square gap to existing path. ONE real port defect found and fixed, shim-side: ll_canvas.js rounded each relative-mouse delta independently while re-anchoring on the real pointer, so on any canvas not displayed at exactly 640x480 the game's cursor lost a fifth of every movement and never caught up (measured 12 of 16 pixels; a 200-px sweep ends three isometric squares short). No LEGOLAND/*.c change; relocs --all 0, 3281/42, wasm 24/24, native 17/17. Open for PORT-M14: the eraser over a ride's own pad removed the RIDE (g_sel_def-shaped destroy cursor); four shipped linkable classes put their entrance off the clearance ring, two of them INSIDE the footprint, on levels that script `LINK ALL`. Notes `docs/lanes/scope-port-m13.md`**
