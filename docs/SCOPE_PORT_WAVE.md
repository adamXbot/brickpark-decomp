...
> **PORT-B3 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-B3)** — the ten hand-written type-3 RLE sprite painters (`LEGOLAND/rlepaint.c`, `LEGOLAND/rlepaint2.c`) ported to C in their `#else` arms so `ShowTitleScreen -> PrintSprite -> RenderSprite -> SoftBlitSprite -> SoftBlitRLEPlain` can draw. Notes `docs/lanes/scope-port-b3.md`
