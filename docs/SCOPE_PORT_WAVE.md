...
> **PORT-B5 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-B5)** — the
> remaining inline-asm bodies ported to C: PORT-B3's `row:` label fix plus
> `SoftBlitAnim` (softblit.c), tri3d.c's four triangle rasterisers,
> `ZBufferHelper` (bigrender.c), `BltAdvisor`/`ShowCapacityOverlay`
> (blitmisc.c), `RenderTransSprite` (popup.c), and a ruling on the three
> ST(0)-ABI helpers (`FastSqrt`/`FastRSqrt`/`sub_458930`). Notes
> `docs/lanes/scope-port-b5.md`.
