...
> **PORT-A6 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A6)** — the split-record class closed at the source: `portable/tools/cdecl.py` computes every object's extent from the game's own `typedef struct` definitions (MSVC x86 layout, `#pragma pack`, ILP32), so `STRUCT_EXTENTS` becomes a regression fixture instead of the mechanism. Notes `docs/lanes/scope-port-a6.md`
