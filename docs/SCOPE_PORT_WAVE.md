...
> **PORT-B4 — Status: READY FOR MERGE (2026-09-12) — THE FRONT END COMES UP: `PLAYER DETAILS`, the 8-slot profile screen, renders complete and the game's own loop runs it at 33.5 fps (the 28 ms flip floor, i.e. at its ceiling). dsound.c is a real silent IDirectSound with a wall-clock play cursor — the old DSERR_NODRIVER made `-nomusic` UNREACHABLE, because InitSoundSystem returns before InitMusicSystem and nothing then writes g_music_disabled; ole32 CoInitialize/CoCreateInstance added (REGDB_E_CLASSNOTREG = the game's own no-DirectMusic path). Page takes `?args=`, `?trace=1` shows fps. INPUT IS DELIVERED (30 DirectInput state reads over 30 frames with a button held) AND IGNORED by the game — six causes ruled out, the remaining chain named. Blockers: CreateThread refusing hangs music-ON (PORT-A kernel32.c, fix measured); `_findclose(-1)` traps the module (PORT-A msvcrt.c:204); 41 unwritten GAME functions, front-end ones are llidb_odf.c + screen.c WindowProc + 8 in screens3.c (matching); gen_link has no log-and-continue trap mode. Notes `docs/lanes/scope-port-b4.md`**
