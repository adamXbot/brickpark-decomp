...
> **PORT-A7 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A7)** — B8's blocker B2 in the generator: a pointer FIELD of a declared struct is a pointer word too, so every such word whose target lands inside an emitted object is re-pointed, and `raw pointer words` is a gate in the manifest. Notes `docs/lanes/scope-port-a7.md`
