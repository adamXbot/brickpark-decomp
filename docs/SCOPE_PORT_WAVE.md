...
> **PORT-A9 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A9)** — the gate B11 §3 asked for: a DECLARATION-INDEPENDENT raw-word census in the manifest (every emitted object, not only the ones the declaration scan visited), M10's by-value-struct sweep promoted to `portable/tools/bvstruct_sweep.py` with a selftest and a ctest, a conservative unbounded-array extent rule in gen_link, and a `--probe-audio` headless mode that counts CreateSoundBuffer calls. Notes `docs/lanes/scope-port-a9.md`
