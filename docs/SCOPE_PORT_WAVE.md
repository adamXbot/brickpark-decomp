...
> **PORT-M5 — Status: READY (2026-09-12)** — the port lanes' recovery findings closed and the LAST THREE asm bodies ported: census **asm stubs 6 -> 3** (the three left are PORT-B5's unreachable ST(0) helpers, so every other inline-asm body in the game now has a C fallback). Five name disagreements settled from the disassembly (0x0046b700 = `ShowStepHint`, 0x0048a800 = `ResetFreePlayTable`, 0x0048f9f0 = `SaveFrontEndState`, 0x00474750 = `CloseActiveThemeButton`, and 0x00443dc0 = `SetVidAnim` — the shipped debug breadcrumb's own name). PORT-M3's three open callback slots closed, including a WRONG CALLEE NAME the byte gates could not see: coaster8.c's `detach` slot is 0x004273f0 `RouteSeat_ReleaseCar`, not 0x004273e0 `RouteSeat_DetachCar`, so the portable build had stopped releasing cars. PORT-B5's texel formula fixed in tri3d.c's and texture.c's headers, and its `sub_458930` finding audited end to end: 111 call sites, 95 of them now round under `LL_FISTP` in the portable arms, and the shipped `g_recip[]` is ROUNDED (g_recip[30] is 2185, not 2184 — `tri_raster`'s expectation is corrected). `TrackShade_FillPoly`, `Span_FillShade` and `Span_FillShadeZ` ported with `coaster_span` (40 checks). 37 files, audit PASS with all 425 `[OK]` rows byte-exact, relocs 0 MISMATCH per file and tree-wide, 3281/42; ctest 9/9 native, 15/15 wasm32. Notes `docs/lanes/scope-port-m5.md`
