...
> **PORT-M1 — Status: READY TO MERGE (2026-09-11) — the prototype conflicts: wasm-ld signature mismatches 130 -> 19 distinct symbols, census 542 -> 437, every change an `#ifdef LEGOLAND_PORTABLE` arm (audit.py PASS on all 156 touched files, relocs 0 MISMATCH, 3281 exact / 42 WIP unchanged). The spine and the browser page now run through the loader, the front-end init and `ShowTitleScreen` to `RLEPaintHit` (rlepaint.c:1016), an UNPORTED INLINE-ASM body — the next blocker and the asm-porting lane's. `legoland_tests` 5/5 under node (loadpos closed). The 19 left all need a call-site change: six struct-by-value-vs-flattened-int pairs, six float-bit-pattern levers, `NewScriptEvent` (two different functions at 0x00468910 / 0x004689f0 sharing one name — levelkw.c and movie3.c call the wrong one), `PtInRect` (cursorseg.c vs PORT-B's user32.c) and `printf` (a gen_link.py alias defect, PORT-A's). Notes `docs/lanes/scope-port-m1.md` §7 has the recipe for each**
