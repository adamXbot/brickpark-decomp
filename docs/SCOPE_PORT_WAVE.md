...
> **PORT-A4 — Status: READY FOR MERGE (2026-09-12) — the last wasm-ld signature mismatch closed in gen_link.py (a CRT alias target is typed from libc's prototype, not from the alias: `CRT_PROTOS` + `defined_here()` + per-argument casts instead of a function-pointer cast); mismatch lines 5 -> 0, distinct symbols 1 -> 0, binaryen validator errors 0 on all 7 modules, 11 forwarders typed from libc. `name_trap.py` names a `call_indirect` type mismatch — caller, the call site's expected type, the candidates ranked by type distance and narrowed to what the rebuilt globals store, and the callback-table global to re-declare — plus `--table` and `--at 0x...`; gen/manifest.md publishes the 80 cast forwarders as the static counterpart. CI gained `portable-wasm`: all five wasm32 targets link (a signature-mismatch grep is a gate) and 5 ctests run with NO game assets, where tests.cmake used to register none. `headless_spine` asserts the first present — 640x480, 301157/307200 non-black, frame checksum 0x4a092b01 — and its skip list lost `loadsprite`. Native ctest 5/5, wasm32 10/10, census unchanged (no game source touched). Notes `docs/lanes/scope-port-a4.md`**
