...
> **PORT-A8 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A8)** — the generator and tools lane: `cdecl.py`'s declarator parser (M8-3), the left-raw words and conflicting wasm signatures attributed in the manifest, M8's slot sweeps and M6's multi-address extern sweep promoted into `portable/tools/` with a ctest, and `name_trap.py` taught the table-index and memory-access trap kinds (the B9-4 class). Notes `docs/lanes/scope-port-a8.md`.
