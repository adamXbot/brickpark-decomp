...
> **PORT-M2 — Status: LANE COMPLETE, awaiting merge (claimed 2026-09-12 by PORT-M2)** — the 19 call-site signature mismatches are down to **1**, and that one is not in the game (`printf`, a `gen_link.py` alias defect, written up with the fix in the notes). 25 game files under `LEGOLAND_PORTABLE` arms: 8 struct-by-value-vs-flattened pairs, 8 float-bit-pattern levers, and **two names wired to the wrong function** — `NewScriptEvent` in levelkw.c/movie3.c was gameframe2.c's `AddScriptString` (0x004689f0, not 0x00468910) and `DrawSupportModel` in coaster4.c was `Coaster3D_DrawModel` (0x00420e90, not 0x00429490); both renamed for BOTH builds, both invisible to every byte gate. Closing the mismatches un-masked a pre-existing defect that made the browser module fail wasm validation (a generated cast forwarder that binaryen directizes) — fixed in coaster10.c. audit PASS x25 (366 [OK] / 6 [WIP]), relocs 0 MISMATCH, /W3 clean, marker sets identical, 3281/42 unchanged, ctest 8/8, census 437 -> 420. `name_trap.py` reports no prototype conflict: headless and the browser page both run to `RLEPaintHit` (rlepaint.c:1016), PORT-B3's. Notes `docs/lanes/scope-port-m2.md`
