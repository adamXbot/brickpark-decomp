...
> **PORT-P2 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-P2)** — TUTORIAL LESSON 2 IS UNWINNABLE AND LESSON 3 IS UNREACHABLE, on one silent defect: `ClampScrollToMap` reads `g_view_left/top/right/bottom`, which scrolltick.c declares at 0x004b95f4 (value 0x0003b600 x4 in the shipped exe, checked against the original's own operands at 0x00461290) but coaster3d.c/coaster10.c/unref3.c declare at 0x008299ac — one name, two addresses, and in a single-namespace build the coaster's 3D span clip wins. The map scroll is 475 px too tight on every edge (`x_max = 4096*mapw - 250880` instead of `- 129280`, confirmed to the unit on both ONE.MAP and TWO.MAP), so Lesson 2's hedge pen holding all four Gardeners sits at game x = 800 on a 640-px screen and can never be reached — by autoscroll or by the MAP screen's jump. P2-1 is not alone: a scan of all 2675 address-commented externs finds **17 names declared at two different addresses**, each one a translation unit silently reading another's object (`movie.c`'s `g_avi_open_count`, `fpui2.c`'s `g_popup` interior alias, `roads.c`'s `g_road_tiles`, `coaster.c`'s `g_tile_sprites`, ...) — a PORT-M sweep, invisible to every existing gate. Second blocker P2-2: a race between the side panel's scroll animation and MAP mode loses the LEGOLAND theme button permanently, ending any level. Lesson 1 (the gate) plays start to finish; B10's PARK-1 and PARK-3 are both gone and A7-2 stays closed. Three replays, no trap all lane. Notes `docs/lanes/scope-port-p2.md`
