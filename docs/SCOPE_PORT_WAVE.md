...
> **PORT-A4 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-A4)** — the generator and harness follow-up: the last wasm-ld signature mismatch (`printf`, typed from the alias instead of from libc), `call_indirect` type-mismatch traps named by `name_trap.py`, a wasm32 CI job, and `headless_spine` as a first-present pixel gate. Notes `docs/lanes/scope-port-a4.md`
