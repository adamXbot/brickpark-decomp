...
> **PORT-M15 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-M15) — DONE, awaiting merge: TUTORIAL LESSON 4 NOW PLAYS TO THE END (9/9 objectives, level complete, debrief, profile advanced to lesson 5) — P3's dual-address class closed 17 -> 0. The address comments were RIGHT all 34 times (relocs.py forced unresolved prints what the original binary targets): never a bad comment, always two objects wearing one name — 9 of them already named elsewhere in the tree (`g_ui_flags`@0x008003e8 is `g_edit_cursor.flags`; `g_tile_sprites`@0x0082c680 is ridecb8's `g_roads_data`; `g_snd_click`@0x004b929c is fpui3/screens3's `g_snd_close` = g_game_fx[9] "Click01.wav"; `g_screen`@0x00668078 is blitmisc's `g_surface_78`; the three carousel/balloonz "+4/+8 shears" are screencb2's SCALAR slots beside the class tables), 5 given a new name (`g_view`->`g_view_cfg`, `g_road_tiles`->`g_road_tile_codes`, `g_frame_ticks`->`g_zb_frame_ticks`, two literals differing only in the case of "on" split, `g_avi_open_count`->`g_advisor_avi_open_count`). P3-1: scrolltick's eight `g_view_*` are SHIPPED SLACK in .data, not a view — renamed `g_scroll_slack_*`; the clamp now settles 475 px further east **to the unit** (g_scroll_x -29696 -> 91904) and the Mechanic's Hut is on screen. P3-2: fpui2's `g_popup` is bighelp's record from +0x1c (InitPopUpInfo reaches both bases in ONE body) — renamed `g_popup_info`; hiring works, 30 bricks a worker, gardener and mechanic. Collateral: `g_cursor_sprite_put[5]`->`[1]` (it had swallowed the four 243200s), `g_lls_goback_on_tut` 44 -> 20 bytes (it had swallowed a whole string). 16 files, audit rows BYTE-IDENTICAL on every one, relocs 0 MISMATCH, 3281/42, rawwords unchanged, wasm ctest 24/24 + native 17/17, ~35,500 frames at 34.2 fps with 0 traps. Open: P3's sweep belongs in the round gate (extern_sweep says "closed" and is blind to this shape). Notes `docs/lanes/scope-port-m15.md`**
