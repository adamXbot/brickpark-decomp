...
> **PORT-M10 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-M10)** — PARK-1 AND PARK-3 ARE ONE BUG AND IT IS FIXED: a by-value struct parameter spelled as a struct in one TU and a scalar in the TU that defines the body is the same dword on x86 and a POINTER on wasm32, and because both lower to one i32 parameter wasm-ld warns about nothing, linkreport's conflict vote sees nothing and nothing traps -- the callee reads a shadow-stack address. `AddObjectToBuildList` (popup.c BPos vs sweep1.c short) put the Space Tower's map tile in the build slot as 0x0a72, so its ObjInst key and its TowerRec both read (114,10): EventTick_Link's square is off the 84x84 map for ever (PARK-1) and SpaceTower_FindRecord never finds the ride (PARK-3). Two more of the class fixed (FreeBuildSlotAt, GetBuildAnimFrame), two recorded for the integrator (StandardRemoveObject, RemoveAllBlokesFromRide -- the dangerous direction). New sweep `tools/port_m10_bvstruct_sweep.py` -- the only gate that sees this class. A8-1 closed: wasm signature conflicts 29 -> 0. M9-3 decided: 0x004b5b3c is `g_cmd_write`. PARK-2 is shim-side (the panel does not claim the hit) -- PORT-B11. `llPark().numVisitors` reads 0x00832bd0, which is `g_power_supply`, not a visitor count. Notes `docs/lanes/scope-port-m10.md`
