...
> **PORT-M14 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-M14) — the eraser taking the RIDE off a pad square is THE GAME’S OWN RULE, correctly ported (M13 §5b closes as not-a-defect). A ride’s pad is two things that look like one: `RefreshObjList` (0x0045d770, 201i/743B, mismatch 0) paves the one-cell RING with `ClearCellForPath` + `AddPathTileGFX` + **`AddPathSquare`** (real path, owned by nothing) and the footprint INTERIOR with `AddPathTileGFX` **only** (path graphics on cells that stay the ride’s). Measured on the Space Tower at base (63,35), 7x6: interior flags 0x90 owner “Space Tower Ride” no path square; ring 0x10 no owner has a path square; a hand-laid path square 0x18 owner “Path” base=itself. `HandleMapClick` (0x00457a70, 751i/2889B, mismatch 0) promotes on `flags & 0x88`, so the three clicks are: ring → nothing (cursor red, error 1), main path → that square only (both neighbours still 0x18), interior → the ride, +40 refund. M13’s suspect `BasicObjectDCalcCursor` is CLEARED: `g_sel_def` is zeroed and reassigned from the pointed cell’s owner in the same call before the +0x94 hook runs, and off an object cell the hook is never reached. The shim’s per-pixel sprite hit is also innocent — a screen-space sweep draws a footprint-shaped rhombus, not a bounding box. No `LEGOLAND/*.c` edited; relocs --all 0, 3281/42, wasm 24/24, native 17/17. Notes `docs/lanes/scope-port-m14.md`**
