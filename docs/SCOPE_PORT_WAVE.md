...
> **PORT-M3 — Status: READY FOR MERGE (2026-09-12) — the typed-callback pass: every function-pointer TABLE and slot the game calls through, censused against the real wasm signature of every body stored in it (475 indirect call sites, 185 game-side, over 30 slots/tables). 26 of 30 now have ONE wasm type, up from 17: the Icon input slot (17 two-argument handlers), the level keyword table (22), four ObjDef slots (61 registration-site adapters), coaster10.c's two geometry vtable casts and TrackDesc's build hook. 15 files, audit PASS 363 `[OK]` / 3 `[WIP]`, 0 REJECT/FAIL, relocs 0 MISMATCH, 3281/42; census 420 -> 411; ctest 8/8 wasm + 3/3 native; `portable/tests/test_callback_types.c` type-checks 454 (slot, body) pairs at compile time. The browser page now draws the title screen. Notes `docs/lanes/scope-port-m3.md`**
