...
> **PORT-B8 — Status: IN PROGRESS (claimed 2026-09-12 by PORT-B8)** — **B2 NAMED, and it is PORT-A's**: the page's heartbeat (`?beat=<ms>`, `ll_host_beat`, a 32-entry ring dumped on WRAP — the only signal that survives a wedged main thread) proves the loop makes NO host call; it is `while (KillSprite(g_low_markers[i].lit) == 0) ;` (screens3.c:1320) with `.lit` NULL because `gen-browser/globals.c` emits the sprite's NAME as the raw x86 address `0x004bed40` — the string is INSIDE the same 600-byte object at +0xa0, swallowed by A6's extent widening, so it has no symbol to re-point to. `g_level_markers` has the same 18 words wrong; those two tables are BOTH doors into the park. Census: **1211 raw-VA words in 177 objects** (g_fp_table 135, kThemeSame 102, g_level_db_sections 91, g_power_table 63, g_lowlevel_ai 59 — all tables the park reads), script in the notes §9. Park NOT reached; front end 32-33 fps, 0 traps. Profile save/load works in-session (`/gamedata/profiles/Profile1.txt`, 272 bytes) — the `"cannot open output file"` lines are ScanForProfiles probing seven absent slots in READ mode, not a fault; MEMFS still loses it on reload (IDBFS, next B lane). B7's second wedge does not reproduce. Notes `docs/lanes/scope-port-b8.md`**
