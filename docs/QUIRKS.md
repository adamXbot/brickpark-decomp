...
| Q6 | left: nothing to fix toward (the DirectMusic lane) | the music plays now, and Egypt → Inca plays `IEtran2`, measured in the tab (`portable/README.md`, the DirectMusic lane). The disc has no `EItran2.sgt`, so the one-string fix has no file to point at. Also played as shipped: `ELtran2.sgt` uses the `EItran2` styles, so `ELtran2.sty`/`ELtran2a.sty` are never heard |
