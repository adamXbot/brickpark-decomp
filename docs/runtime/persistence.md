...
| --- | --- | --- |
| 1 | u32 element count; each `{u32 name length, name bytes, u32 flags & 0x3000e}`; u32 distinct TSF count and length/name pairs | [savegame.c](../../LEGOLAND/savegame.c) |
...
| `9c`, `a0..a8`, `ac/b0`, `b4/b8` | Person`08`, position`10..18`, screen`1c/20`, pair`24/28` |
| `bc/c0/c4/c8`, `cc..d4`, `d8/dc/e0` | Person`34/30/38/3c`, scale`40..48`, `4c/88/54` |
...
This field map follows the staging declaration; names do not imply that the meaning of every saved value is known. [savegame.c](../../LEGOLAND/savegame.c)
...
| `80`, `84..8f`, `90/94`, `98..a3` | Person`08`, position`10..18`, screen`1c/20`, scale`40..48` |
...
Script strings are signed length (-1 means null) followed by exactly length bytes; the recovered reader allocates length+1 and adds NUL locally. This contradicts the older `savechunks.c` header's length+1-on-disk statement. The writer `0x0046c620` remains external in this snapshot, so reader framing is the direct evidence and the writer side remains a named check for later recovery. [savegame2.c](../../LEGOLAND/savegame2.c), [savechunks.c](../../LEGOLAND/savechunks.c)
...
Profile files `profiles/Profile%d.txt` and save headers `%dsave%d.sh` hold one packed `0x110` record. Offsets: name region`00..1f`; setting`20`; save flag byte`24`; volume/settings dwords`28/2c/30`; 15-byte stats`34..42`; **200-byte block at43**, ending10a; tail dword10b; final byte10f. `saveprof.c` prose claiming block+a3 is a typo: its declaration and `profiles.c` both put it at43. Name interpretations differ (30 bytes plus two flags versus32 name bytes), so valid name length is not settled solely by these views. [profiles.c](../../LEGOLAND/profiles.c), [saveprof.c](../../LEGOLAND/saveprof.c)
...
All six save/profile source files were read for their headers, layout declarations and relevant serializer notes. Exact bulk field maps are complete for container, script events, profiles, path squares and order records; visitor/worker flattened payloads retain fields whose gameplay meanings are unknown. No original save round-trip was run in this documentation-only scope. [savegame.c](../../LEGOLAND/savegame.c), [savechunks.c](../../LEGOLAND/savechunks.c), [savechunks2.c](../../LEGOLAND/savechunks2.c), [savegame2.c](../../LEGOLAND/savegame2.c), [saveprof.c](../../LEGOLAND/saveprof.c), [profiles.c](../../LEGOLAND/profiles.c)
