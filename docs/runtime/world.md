...
The MoveLine loader calls its inputs whole tiles and shifts them by eight bits, while several ride-call-site structs label their world/target inputs24.8. The shared VA is established, but the coordinate interpretation at that boundary is inconsistent in the current reconstruction. `NavigMoveLine` advances position without a target-arrival test; the low-level state7 completion body is absent. Do not infer exact destination snapping from these helpers alone. [bnvmove.c](../../LEGOLAND/bnvmove.c), [goldrush2.c](../../LEGOLAND/goldrush2.c), [bigsim.c](../../LEGOLAND/bigsim.c)
