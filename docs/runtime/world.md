...
This page consolidates the recovered contracts at the Scope J baseline. All36 primary sources are documented, including the original-data AI recovery. Coverage tracks recovered contracts separately from compiler matching. All offsets describe the original 32-bit records.
...
The separate ride walk-path builder consumes an eight-byte PolyLine `{count, segment-delta pointer}`. It allocates one eight-byte WalkPath `{node count,node pointer}` followed immediately by twelve-byte nodes `{int x,int y,int state}`; state starts at zero. The node count is the sum of each delta's truncated Euclidean length. This representation is distinct from both rectangular path squares and the flume's floating-point path. [mappath.c](../../LEGOLAND/mappath.c)
...
`BuildWalkPath` begins at `(0,0)` and expands each segment using integer division, then advances the origin by the full delta. The original accumulator order emits the segment origin twice when its length is at least two; its final emitted point is only `(length−2)/length` along that segment. The endpoint becomes the next origin and is not emitted as this segment's endpoint. Zero-length segments emit no nodes. Failed allocation skips initialization but still enters any nonempty interpolation, causing a null dereference. The earlier header's idealized kth-point formula must be read with its subsequent off-by-one correction. [mappath.c](../../LEGOLAND/mappath.c)
...
| `+0c/+0e/+10/+1c` | plan u16 / low-level state u16 / pending state u16 (also called step) / scratch timer | [blokelist.c](../../LEGOLAND/blokelist.c) |
...
| `+60/+62/+64` | action byte / flags word / movement-event byte | [bigsim.c](../../LEGOLAND/bigsim.c) |
| `+68/+6c` | world x/y, 24.8 | [workers3.c](../../LEGOLAND/workers3.c) |
| `+72/+73/+74/+75` | heading / line angle or requested heading / AI phase / walk delay | [workers.c](../../LEGOLAND/workers.c), [simcore2.c](../../LEGOLAND/simcore2.c) |
| `+78/+7a/+7c/+80` | signed stay timer / signed mood / u16 hunger / hunger increment byte | [simcore2.c](../../LEGOLAND/simcore2.c), [workers3.c](../../LEGOLAND/workers3.c) |

Ride SeatSlots are 20 bytes in the class `+cc` doubly linked chain, binding a Bloke at `+08`, seat at `+0c`, and Person3D at `+10`. The `+10` Person3D interpretation follows the explicit `Bloke->person` assignment in `savegame.c`; `blokelist.c` calls it a ride owner, so that header label is retained as a disagreement. Render-list insertion takes the slot, not the Bloke. MoveLine is 12 bytes `{int x,y; short dx,dy}` whose accumulator adds eight fractional bits to caller coordinates; world-coordinate callers supply24.8 and receive24.8 output. [Original arrival/scale proof](core-data.md#movement-arrival-and-coordinate-units) [blokelist.c](../../LEGOLAND/blokelist.c), [savegame.c](../../LEGOLAND/savegame.c), [bnvmove.c](../../LEGOLAND/bnvmove.c)
...
Hunger tiers (called “tiredness” in the older worker header) are `{1000,2400,4000,7000}`. At >=7000, a visitor outside flag mask `0x28` receives plan 3 (go home). Otherwise hunger can increase by `+80` every 32 ticks while <=7000; a rider exactly at 7000 can still increase. Disabling `0x00832990` skips this check and makes the tier helper return 1. Mood changes by `adjustment[event] * scale / 100`, then clamps to [-30000,30000]. [workers3.c](../../LEGOLAND/workers3.c), [simcore2.c](../../LEGOLAND/simcore2.c)
...
`CalcMoveLine` initializes its accumulator to `(input<<8)+0x80`; ride inputs are already24.8 world coordinates, so the internal accumulator adds eight fractional bits. Its output shifts back by8. Unit velocity is atan2-derived, scaled by 256 and rounded using floor(v*256+0.5). `CalcMoveLine` returns a signed integer angle measured in256 units per turn (callers commonly narrow it to a byte); Bloke heading`+72` uses octants0…7, with8 meaning no heading. Byte`+73` holds the wrapped line angle until the turning helper replaces it with the requested octant; state5 consumes that latter form. [AI field evidence](ai-data.md#shared-fields-motion-and-selection-contracts) Random walking follows an available nonreverse exit at corners, excludes reverse and its neighbours at junctions, and wanders at dead ends. Turns save prior low-level state at `+10`, enter state5 and reset delay. Off-map is an obstacle/path edge; a person currently on RF-blocked terrain may leave it. [bnvmove.c](../../LEGOLAND/bnvmove.c), [workers.c](../../LEGOLAND/workers.c), [pathbuild.c](../../LEGOLAND/pathbuild.c)
...
The [high-level AI contract](ai-data.md) covers all26 plan slots, including park entry/exit, ranked destination selection, routing/admission, Cafe Brolly reservations, timed visits and worker return plans. Its action and route-result tables specify all fall-through, retry and same-tick transitions. The [low-level AI contract](ai-low-data.md) covers all16 states: delays, random/path/line walking, turning, special-tile waiting, worker motion and cursor-held waiting. States use a single pending word, so nested transitions overwrite it; restoration clears that word. High-level table labels use hexadecimal; low-level state labels use decimal. [Dispatch source](../../LEGOLAND/blokeai.c)

Movement events at`+64` distinguish obstacle mask1 and nonwalkable mask2 before the high-level dispatcher clears the byte. Movement validates proposals after advancing the internal line accumulator, so a rejected step can leave the accumulator ahead of the stored world position. Special-tile entry/status/leave callbacks and their state9 resumption rules are fully described in the low-level evidence. [Low-level transitions](ai-low-data.md), [high-level routing](ai-data.md)
...
LEGO appearance palette at `0x004b7ac0`, RGB triples: 0 `#000000`,1 `#007bc6`,2 `#732910`,3 `#008c4a`,4 `#bdcede`,5 `#f71821`,6 `#ffffff`,7 `#ffd600`. Departure scores enter a 25-byte ring at `0x00832bb0`, index `0x00832bc9`, bucketed by runtime thresholds, reset to−8000,100,1000,5000; mood display boundaries reset to−1000/1000. Scripts can replace all five thresholds. [Defaults and override proof](core-data.md#mood-hunger-and-departure-defaults) Worker refunds are 30 bricks. [blokelist.c](../../LEGOLAND/blokelist.c), [workers.c](../../LEGOLAND/workers.c), [blokemisc.c](../../LEGOLAND/blokemisc.c)

The appearance transfer block uses `0x004b59bc` for the Bloke pointer, `0x004b5988` normalized sex, `0x004b598c/90` expanded leg/arm RGB values, `0x004b5994` the twenty-byte chest-name field and `0x004b59a8` the face-name field. Color getters expand palette indices from Person3D `+8c/+90` to `0xRRGGBB`; the material builder defaults to palette index3. These values are runtime appearance state, not extra fields in the Bloke allocation. [blokelist.c](../../LEGOLAND/blokelist.c)

### Original bugs and limits

Texture getters discard their comparison and always return `chest girly2`; invalid sex leaves the lookup pointer uninitialized. Type-3 retargeting writes tile units into a normally 24.8 target. Mood-event indexing is unchecked. The complete dispatch tables are in [core data](core-data.md); original high-/low-level handler contracts supplement the source-only subset. [blokeai.c](../../LEGOLAND/blokeai.c), [simcore.c](../../LEGOLAND/simcore.c), [simcore2.c](../../LEGOLAND/simcore2.c)

The original selection plan reads element`+1c` after storing a class's LLIDB element, so it tests the next element's flags for ordinary interior records. Failed Join after entering null plan5 can leave the visitor stalled; Join itself leaks its allocated20-byte slot when no instance matches. Cafe reservation and flag8 cleanup occur only on specific successful actions. Low-level state8 uses an unsigned nonzero height test, state9 can restore twice and recursively dispatch, and tile-leave code checks the enter pointer before calling the unchecked leave pointer. PathSquareStop passes raw24.8 coordinates to a tile-coordinate lookup. These are recorded operations, with no claim about their frequency in normal play. [High-level defects](ai-data.md), [low-level defects](ai-low-data.md)
...
Each category has published working count at `+00`, available-definition count at `+04`, published staff/capacity at `+08`, published occupied-cell count at `+0c`, salvage value at `+10`, income coefficient at `+14`, cap coefficient at `+18`, and accumulating cell count/working count/value/staff at `+1c/+20/+24/+28`. The body corrects the local declaration comments that reverse the published counts at `+00/+0c`; its no-map branch counts definitions into `+04`, not the header's `+14`. “Staff/capacity” retains the consumer's role without asserting one universal meaning for every object's `+2e`. [bigsim.c](../../LEGOLAND/bigsim.c)
...
Path cells increment category0's occupied, working and staff accumulators. Other qualifying object cells count footprint area, but working count/value/staff accrue only on the running object's anchor. Publication copies the four accumulators to `+0c/+00/+10/+08`, then divides category0's published staff by100. Income uses category0's occupied-cell contribution, multiplied by its `+14` and divided by100, capped by its `+18 × 100`; categories1,4,5 each contribute `+14 × published staff`, capped by their own `+18 × 100`. The sum is divided by100, raised to the configured minimum, lowered to the maximum, then capped by map `+1a`, in that order. Categories2/3 have no direct term. Without the map-screen bits, only categories1…5 definition counts are reset and eligible definitions with nonzero `f8` are counted; category0 is retained. [bigsim.c](../../LEGOLAND/bigsim.c)
...
Power table `0x004b9340` has 65 eight-byte `{name,power}` entries, case-insensitive, terminated by an **empty name**; the [complete original table](core-data.md#power-table) now supplies all values, including Small Power Station +800, Crystal Power Station +2500 and Castle Obj−400. Power globals: percent`0x832bcc`, supply`bd0`, demand`bd4`, unserved`bd8`, unserved count`bdc`. [power.c](../../LEGOLAND/power.c)

Nearby-broken-cell table `0x004bff28` has 12 offsets: cardinal one-step, two-step cardinals, diagonals; inspection reads condition from the anchor cell. Message table `0x004ba8e0` has 17 `{topic,cooldown,last shown}` records; only a higher-ranked message past cooldown is shown. The complete original [cooldown values](core-data.md#message-cooldowns) and [ordered offsets](core-data.md#nearby-cell-scan) are now extracted. [workorder2.c](../../LEGOLAND/workorder2.c)
...
`IsObjectRunning` dereferences the result of a bounds-checked map lookup even when it is null. The route relaxation branch is entered only for fresh state0, so its nested state1/state2 handling is unreachable in the recovered control flow; do not silently replace this with a conventional revisiting A* relaxation rule. [sysmisc3.c](../../LEGOLAND/sysmisc3.c), [simcore.c](../../LEGOLAND/simcore.c)
...
`ClampPopUpToScreen` keeps x at least130 when panel mode is not2, top at least37, and bottom at most367; its return value can still be the computed bottom limit when the original y is retained. Popup text measurement acquires a DC without releasing it. Work-order previews and LLIDB selection-dialog behavior are consolidated with their [presentation consumers](presentation.md). [misc3.c](../../LEGOLAND/misc3.c)

Bloke `+7c` is the hunger accumulator: the food-venue reset, UI tier and original goal strings reconcile the older visitor “tiredness” alias. [Consumer evidence](core-data.md#mood-hunger-and-departure-defaults) [simcore2.c](../../LEGOLAND/simcore2.c), [workers3.c](../../LEGOLAND/workers3.c), [rides.c](../../LEGOLAND/rides.c)

MoveLine accepts the caller’s coordinate unit and adds eight fractional bits internally; ride callers pass24.8 world positions. `NavigMoveLine` only advances, while original state7 performs the separate twice-speed arrival test and restores the saved state without snapping. [Instruction proof](core-data.md#movement-arrival-and-coordinate-units) [bnvmove.c](../../LEGOLAND/bnvmove.c), [goldrush2.c](../../LEGOLAND/goldrush2.c), [bigsim.c](../../LEGOLAND/bigsim.c)

## Original-data recovery

The [core evidence](core-data.md) supplies all280 visitor names,65 power values,17 cooldown records,12 ordered neighbouring cells, the26/16 AI dispatch tables, mood defaults/override, hunger field reconciliation and state7 completion. These replace the earlier named external-table boundaries. State7 restores the saved state within twice-speed distance and does not snap position. The hunger accumulator is reset when a visitor leaves a food venue, confirming its operational meaning.

The two AI evidence pages close the former handler boundary with47 bounded function bodies:19 high-level/admission ranges and28 low-level/helper ranges, including one independently checked source-covered helper. Their dispatch, branch and byte checks are published alongside the complete state transitions. [High-level evidence](ai-data.md), [low-level evidence](ai-low-data.md)
