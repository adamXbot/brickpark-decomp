...
This page consolidates recovered runtime behaviour. Addresses and offsets describe the original 32-bit executable; padding and unidentified fields remain unidentified. “Documented” means the recovered mechanics of that source are covered, not that its C is fully matched or that every asset byte has been recovered. Original binary/data recovery closes every previously partial transport row; preserved unused fields and unchecked inputs remain explicitly described. The callback registration reference is [RIDE_CALLBACKS.md](../RIDE_CALLBACKS.md); its older slot descriptions must be read with the actual per-class usage described here.

## Coverage

| Source | Subsystem/material | Status and remaining boundary |
| --- | --- | --- |
| [bswater.c](../../LEGOLAND/bswater.c) | Lake records, launch, flood, carousel tick | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [bswater2.c](../../LEGOLAND/bswater2.c) | Reservation mover, docking, reachability, ancillary ride records | Documented |
| [bswater3.c](../../LEGOLAND/bswater3.c) | Boat animation, tower cars, animation displacement | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [jcroute.c](../../LEGOLAND/jcroute.c) | River DFS and breadth-first flood | Documented |
| [junglecruise.c](../../LEGOLAND/junglecruise.c) | Boats, river placement, decoration teardown, drawing | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [roads.c](../../LEGOLAND/roads.c) | Road tile encoding, river boat animation | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [roads2.c](../../LEGOLAND/roads2.c) | Road allocation and route tree | Documented |
| [schoolcar.c](../../LEGOLAND/schoolcar.c) | Steering, lifecycle, coaster route/pool/save internals | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
...
| [schoolcar7.c](../../LEGOLAND/schoolcar7.c) | Rotation, route reset/position save, rail offsets, LMS fixups | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [schoolcar8.c](../../LEGOLAND/schoolcar8.c) | Vector ops, route seats/hooks, geometry and rendering helpers | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [coaster.c](../../LEGOLAND/coaster.c) | Track graph/editor, relocation save, cars/lights | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [coaster3d.c](../../LEGOLAND/coaster3d.c) | View, geometry, projection and polygon raster submission | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [coaster4.c](../../LEGOLAND/coaster4.c) | Piece constructors, appearance, view, save walk and shadows | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [coaster5.c](../../LEGOLAND/coaster5.c) | Fit, joint wiring/profiling, car construction, sqrt tables | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
...
| [coaster8.c](../../LEGOLAND/coaster8.c) | Joint algebra, route/seat/state/save helpers, palette and raster acquisition | Documented; stale energy/seat labels reconciled |
| [coaster9.c](../../LEGOLAND/coaster9.c) | Model text/drawing, clip-ring, route physics and seat helpers | Documented; all26 bodies and original aliases reconciled |
| [coastertiny.c](../../LEGOLAND/coastertiny.c) | Route/seat/pool/timer/model/raster micro-helpers | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
...
| [lfmisc2.c](../../LEGOLAND/lfmisc2.c) | Queue pop/step, corner coordinate, traffic gate and power-station add handlers | Documented; exact empty-output and callback behavior |
| [logflume.c](../../LEGOLAND/logflume.c) | Class shims, drawing, placement and top-level save | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
| [logflume2.c](../../LEGOLAND/logflume2.c) | Route/shape/probe and shared piece machinery | Documented; original tables/types recovered in [transport-data.md](transport-data.md) |
...
| Lake artwork | 16 rows ×25 byte tile indices; lookup is base of TSM entry 0 plus byte slot. Both original 16×25 tables are byte-identical; all rows and hashes are in [transport-data.md](transport-data.md#shared-water-artwork-and-boat-geometry). A TSM record is 8 bytes `{element, loaded-u16-pointer}` at `0x0082adf4`. [bswater.c](../../LEGOLAND/bswater.c) |
| Boat motion data | `BsStep[4]` at `0x004b5118`, each `{int dx,dy,sx,sy}`; clockwise and anticlockwise `BsCurve[4]` at `0x004b5158/98`, each `{float startAngle,endAngle,offsetX,offsetY}`. Both copies are byte-identical. All N/E/S/W steps and clockwise/anticlockwise arc rows are printed in [transport-data.md](transport-data.md#shared-water-artwork-and-boat-geometry). [bswater3.c](../../LEGOLAND/bswater3.c) |
...
| Record | Layout |
| --- | --- |
| `JcStation`, 0x44 | +00 station square; +02 start square; +04 end square; +08 DFS result stored in a pointer-sized field; +14 queue count; +18 five queued visitors; +2c dispatch timer; +30 three prospective riders; +3c next; +40 accumulated value/takings. +0c initializes9999 and+10 initializes0; these preserved fields have no role in the recovered river/boat consumers. [ridecb9.c](../../LEGOLAND/ridecb9.c) [junglecruise.c](../../LEGOLAND/junglecruise.c), [jcroute.c](../../LEGOLAND/jcroute.c) |
...
| Decorations | All have own/owner packed squares at +00/+02. `JcMonkeyFish` is 0x0c bytes, unidentified +04 dword, next +08, head `0x00629c30`. `JcMonkeyTree` and `JcDeco` allocate8 bytes, next+04, heads `0x00629c2c/34`. Original constructor sizes match the removal views exactly; fish+04 initializes zero. [transport-data.md](transport-data.md#shared-water-artwork-and-boat-geometry) [junglecruise.c](../../LEGOLAND/junglecruise.c) |
...
| Animation tables | Step table `0x004b7148`; clockwise arc `0x004b7188`; anticlockwise arc `0x004b71c8`; same 16-byte record meanings and geometry as boating school. All per-entry values are recovered in [transport-data.md](transport-data.md#shared-water-artwork-and-boat-geometry). [roads.c](../../LEGOLAND/roads.c) |
...
| Queue path/node | Path: +00 count/capacity, +04 points pointer, then `count*12` point bytes in the allocation. Node: +00 next, +04 rider reference. Each 12-byte path sample is int x/y followed by a zero-initialized, preserved dword unused by the position getter. Original constructor/getter evidence is in [transport-data.md](transport-data.md#flume-records-and-overlay-ordering). [logflume5.c](../../LEGOLAND/logflume5.c), [logflume6.c](../../LEGOLAND/logflume6.c) |
...
Placement probes the four compass neighbours into one shared array at `0x004cbe20`. Every probe overwrites the previous result. Cell spacing is2×2 from the static footprint `{0,0,2,2,0}` via `right−left`/`bottom−top`. The per-class ODF rectangles are recovered in [transport-data.md](transport-data.md#original-odf-footprints-and-flume-image-lists). Neighbours with kind 3 or 4 expose free ends; other kinds are removed before shape validation. A legal square has one or two connections. One neighbour extends a route; two neighbours splice routes, reversing same-facing ends when necessary. [logflume2.c](../../LEGOLAND/logflume2.c), [logflume6.c](../../LEGOLAND/logflume6.c)
...
`LFQueue_PopFront` advances the front bloke's action byte, clears flag0x40, returns its rider through the output pointer, unlinks the header through shared0x411f00 and frees the8-byte node. An empty queue leaves the output untouched; `LFRun_Tick` supplies an initial null. The rider pointer is reloaded after the flag write. `LFQueue_StepFront` walks **every** node and calls the one-rider step only for walk-state word+0e equal to0; nonzero states skip that frame. Its separate tx/ty arguments have the same ABI layout as the caller's by-value `Pos`. `LFQuadBottomRight` doubles both tile dimensions in escaped locals and returns `(doubledWidth>>1)+doubledWidth`, likewise y: `(3*tileWidth,3*tileHeight)` for ordinary dimensions. [lfmisc2.c](../../LEGOLAND/lfmisc2.c), [logflume4.c](../../LEGOLAND/logflume4.c)
...
Overlay sets are `{count,frames}`, but their 12-byte frames mean `{unused-by-drawer,cumulativeChildPieceThreshold,sprite}`. Corner1/3 rows are `(0,3,null),(4,5,null)` before sprite installation; hold-up rows are `(0,8,null),(9,9,null),(10,11,null)` with the middle sprite always null. The original helper walks compound-piece endpoints/list links, interleaves boats overlapping each child and draws an overlay when the cumulative child counter reaches frame+4. It never uses frame+0 as x or y and does not reset the counter when advancing frames. It passes caller offsets unchanged. This supersedes the source's misleading `{dx,dy,sprite}` name. Exact table addresses, original instructions and image offsets are in [transport-data.md](transport-data.md#flume-records-and-overlay-ordering). Entrance overlays generally use `(−100,−209)`, except entrance1 `(8,−113)`. [logflume.c](../../LEGOLAND/logflume.c), [lfentrance.c](../../LEGOLAND/lfentrance.c)

The ten track-sprite names at `0x004b4768`, in index order, are `fc1a_m,fc2a_m,fc3a_m,fc4a_m,fs1_m,fs2_m,fe2_m,fe3_m,fe4_m,fe1_m` (all `.lls`); loaded pointers are at `0x004c2abc`. Track creation also loads `LOG FLUME IMAGE LIST`, `LOG FLUME TRACK ENDY LIST`, `fc1_m3.lls` and `fc3_m3.lls`, sets class flag 0x400, clears draw offsets and builds path geometry. Shape lookup uses one overlapping dword array at `0x004b473c`: kind 3 indexes dir, kind 1 indexes 4+dir, kind 2 indexes 6+dir, kind 4 uses dword `0x004b4764`; null/unknown kind returns 0. The original numeric contents are `[6,7,8,9,4,5,0,1,2,3,10]`; all four loaded flume image lists, sprite names and offsets are in [transport-data.md](transport-data.md#original-odf-footprints-and-flume-image-lists). The bases at +0/+0x10/+0x18 are not equally spaced, correcting the header's blanket “eight bytes apart.” [logflume.c](../../LEGOLAND/logflume.c), [logflume2.c](../../LEGOLAND/logflume2.c)
...
- `LFPath_Point(t=1)` reads point n (or−1 reversed) on2-point paths; on4-point paths the rounded quotient and therefore index depend on x87 precision. n=1 divides by zero. [transport-data.md](transport-data.md#flume-endpoint-arithmetic) The header calls these unreachable, but advance only normalizes **>1**, so it does not establish safety for exact equality. All normal tables have 2/4 points; endpoint reachability needs a numeric/runtime test before claiming it impossible. [logflume7.c](../../LEGOLAND/logflume7.c), [logflume6.c](../../LEGOLAND/logflume6.c)
...
`TrafficLightAllows` rejects headings3/7 when the NS global is nonzero, headings1/5 when EW is nonzero, and every heading when both globals arezero. With NS only,1/5 and diagonals0/2/4/6 pass; with EW only,3/7 and diagonals pass; with both set, only the diagonals pass among valid0…7 headings. Other integer headings also pass whenever either global is set. Thus the helper names and header's green-axis wording do not replace the exact exclusion tests. [lfmisc2.c](../../LEGOLAND/lfmisc2.c)
...
There is one castle/coaster at `0x00829ae0`. Its node sentinel begins at record +04. A **0xa4 allocated track piece** contains graph prefix0x2c, footprint0x14 at+2c, cached world Vec3 at+40 and inline0x58 geometry at+4c. The original `GetTrackNodeWorldPos` returns the address+4c for raised geometry, closing the former tail ambiguity. The earlier0x50 node and0x24 footprint declarations were access views, not allocation boundaries. [transport-data.md](transport-data.md#complete-track-piece-and-route-node-storage) [coaster.c](../../LEGOLAND/coaster.c), [coaster4.c](../../LEGOLAND/coaster4.c), [schoolcar.c](../../LEGOLAND/schoolcar.c)
...
| `TrackNode`, 0xa4 allocation | +00 state bits (bit 0 raised, bit 1 ring); +04/+06 16-bit map coordinates; +08 class; +0c descriptor; +10 owner; +14 head joint; +20 tail joint; +2c 0x14 footprint; +40 cached world Vec3; +4c inline0x58 geometry. [coaster.c](../../LEGOLAND/coaster.c), [coaster6.c](../../LEGOLAND/coaster6.c) |
...
| `TrackDesc`, 0x34 payload (ordinary table stride0x38) | +00 raised/anchor flag; +04 tail height h0; +08 head height h1; +0c/+14 two8-byte joint parameter blocks: ordinary masks15/15, castle2/4, all packed coordinate offsetszero; +1c draw, +20 build, +24 query, +28 place, +2c remove; +30 carries-path. The ordinary descriptors' trailing+34 dword is zero alignment/storage; the castle ends at+34, where the following string begins. [transport-data.md](transport-data.md#coaster-descriptors-and-geometry-inputs) [coaster.c](../../LEGOLAND/coaster.c), [coaster6.c](../../LEGOLAND/coaster6.c) |
...
| `RouteNode`, 0xec | +08 front cursor; +40 rear parameter; +44 rear route position; +78/+98 two 0x20-byte seats, occupant at seat +0c; +e4/+e8 previous/next links. `schoolcar.c`'s older +e4 “payload” guess is superseded by seat/node micro-helper evidence. [schoolcar8.c](../../LEGOLAND/schoolcar8.c), [codex-a.md](../lanes/codex-a.md) |
...
`VideoSurfaceInfo` is0x18 bytes: pitch in bytes (32-bit)+00, width+04, height+08, bits pointer+0c, unused+10 and format+14. `ModelImage` is an8-byte data-pointer/length pair. Each0x20-byte `RouteSeat` is local Vec3+00, occupied car+0c and four inline function pointers+10/+14/+18/+1c; these are not a separate vtable pointer. [coaster8.c](../../LEGOLAND/coaster8.c)

`ClipRect` at route node+c8 is 0x1c bytes: left/top/right/bottom at+00/+04/+08/+0c, edge mask+10, previous+14 and next+18. This ring is independent of route links+e4/+e8. `Transform` is 0x30 bytes `{Vec3 position, Mat3 basis}`. Model projection emits16-byte `{x,y,z,clip}` vertices, whereas the swept track mesh uses20-byte vertices with an additional shade field. The new source's temporary `SpanRect` names are copy-only; ring consumers establish left/top/right/bottom order, independently of the span-filler's top/left convention. [coaster9.c](../../LEGOLAND/coaster9.c), [coaster3d.c](../../LEGOLAND/coaster3d.c), [coastermath.c](../../LEGOLAND/coastermath.c)
...
The full route-node layout is flags/kind at+00/+04; front/rear0x38 cursors at+08/+40; seats+78/+98; midpoint Vec3+b8; potential energy+c4; clip-ring rectangle0x1c+c8; previous/next links+e4/+e8. Each cursor has current parameter/RoutePos, two wheel phases, previous parameter/RoutePos. Mode2 selects position/tangent pair1 and its position slot2; the historical tail-tangent name is misleading. Exact setter/draw evidence is in [transport-data.md](transport-data.md#complete-track-piece-and-route-node-storage).
...
The original descriptor table fixes castle masks2/4 and ordinary track masks15/15; endpoint height pairs are castle0/0, flat0/0, raised25/25, low2/2 and path15/15. `TrackJointSloped` returns1 precisely for descriptor.flags==0 and opposite entry/exit headings; predecessor/successor counters stop at bit0 anchors. Exact descriptor hooks and raw-byte verification are in [transport-data.md](transport-data.md#coaster-descriptors-and-geometry-inputs).

`JointOppositeDir` maps1↔4 and2↔8, returns1 for free-end input−1, and0 for every other input. The −1 arm is an original boolean result, not a preserved free sentinel. `TrackJointSloped` tests the entire descriptor flag word againstzero before comparing the exit with that opposite result; a flag with bit0 clear but another bit set still fails. [coaster8.c](../../LEGOLAND/coaster8.c)
...
The loading bay only boards with castle state 2 and a ready dispatch predicate; a closed route without a deadline also blocks boarding. It takes state-1 cars in list order, attaches available seats and changes them to state 2. Boarding at least one car resets the route and arms a 3000-tick deadline; finding no seat before boarding any leaves the timer unchanged. An open coaster instead marks cars older than 5000 ticks state 4 for eviction. Seat initialization sets y=0,z=−4, takes x from a three-entry kind table at `0x004b559c`, and subtracts spacing at `0x004b55a8` for the second seat; kind-x values are `[0,4,8]` and spacing is16. [transport-data.md](transport-data.md#complete-track-piece-and-route-node-storage) New route nodes prepend after the sentinel, and removal refuses the sentinel itself. [schoolcar.c](../../LEGOLAND/schoolcar.c), [schoolcar8.c](../../LEGOLAND/schoolcar8.c)

`RouteNode_Alloc` requests a zeroed0xec-byte record, returnsnull on failure and otherwise initializes the two seats for its kind. `RouteNode_FindFreeSeat` invokes each seat's occupied hook in order and returns the first false seat, ornull. `RouteSeat_InitPosition` copies all three coordinates, clears car+0c and installs query0x4273c0, attach0x4273d0, destruction-request0x4273f0 and frame-draw0x427410. The +18 hook invokes occupied car's destructor+24; separate helper0x4273e0 clears/returns the seat's car pointer. This corrects the new header's loose “detach/update” names without conflating the two operations. [coaster8.c](../../LEGOLAND/coaster8.c), [coastertiny.c](../../LEGOLAND/coastertiny.c), [transport-data.md](transport-data.md#complete-track-piece-and-route-node-storage)
...
`RoutePhys_SetState` finds the route at solver+3c, rewrites route dimension+20 to2, forwards state component0 as raw float bits to position the train, then forwards component1 to the energy setter. `Route_SumCarVelocity` starts at the embedded node and includes it in a do/while sum of every node's +c4 **potential energy**. The new source's speed/velocity names are retained only as function aliases; the derivative and original +c4 producer establish the energy contract. [coaster8.c](../../LEGOLAND/coaster8.c), [coaster7.c](../../LEGOLAND/coaster7.c), [transport-data.md](transport-data.md#complete-track-piece-and-route-node-storage)

`Route_TravelPerTick` evaluates the tangent at the current route position with mode1, current parameter and offset0, then returns `sqrt(dot(tangent,tangent))*speed*0.0015875f`. `Route_TravelThisTick` obtains that speed through0x41dca0: unlike the energy setter, this getter derives `sqrt(2*(energy−potential)/mass)` with a nonpositive clamp. `Route_SumMass` includes the embedded sentinel in the sum of constant0.1f per node. `RouteCar_GetVelocity` is the historical name for the direct potential-energy+c4 accessor. `VecMath_Sqrt` loads the argument into ST(0), calls the selected fast-sqrt hook and spills its result to binary32 before returning. [coaster9.c](../../LEGOLAND/coaster9.c), [transport-data.md](transport-data.md#coaster9-helper-reconciliation)
...
`RouteNode_ClearActive` clears bit0; `RouteNode_IsPending` reads the low signed byte and returns its bit0 as a boolean. These aliases address the same draw-completed flag. `RouteNode_AddPending` skips a set flag or a target unequal to both front/rear current piece pointers at+c/+44. Otherwise it invokes0x41ea70, which draws wheel cursors, the kind-selected train model and both seats, then sets bit0. The lane's “links eligible nodes” wording is misleading: this helper performs drawing, while the separate clip-ring primitives below actually link records. [coaster9.c](../../LEGOLAND/coaster9.c), [transport-data.md](transport-data.md#coaster9-helper-reconciliation)
...
`Coaster3D_SetCarClipDepth` copies the car-view direction into the current view, copies its separate angle and leading scalar a, and stores `(angle+a)/2` and `(a−angle)/2` as the high/low factors. `Coaster3D_DrawModel` requires both model and material pointers. It transposes the basis to transform the view light into model coordinates, scales that light by the low factor, builds the model transform, multiplies view×model and projects all positions with stride0x10. Low32-bit RDTSC elapsed cycles are added to0x4dcbc8 before surface acquisition; acquisition failure therefore still records transform time but skips raster passes. Success calls flat, smooth and textured passes in that order, then the no-op restore helper with the saved surface argument. [coaster9.c](../../LEGOLAND/coaster9.c), [transport-data.md](transport-data.md#lms-lfm-ltx-and-palette-assets)
...
`RouteNode_UpdateClipRect` builds the node transform, computes the bounds of model0x82add0 and installs them at node+c8. `RouteNode_LinkClipRect`/`RouteNode_UnlinkClipRect` pass node+c8 to the ring primitives. `ClipRect_Link` inserts immediately after sentinel0x829a3c: old-next.prev=new, new.next=old-next, sentinel.next=new, new.prev=sentinel, with the original intervening pointer reloads. `ClipRect_Unlink` patches next.prev and prev.next, leaving the removed record's own links intact. No membership or null guards are added. [coaster9.c](../../LEGOLAND/coaster9.c), [coaster3d.c](../../LEGOLAND/coaster3d.c)
...
`CoasterRider_GetSaveIndex` searches class0's +cc rider chain, using the link access prefix `{next+00,unused+04,bloke+08}`, and returns the zero-based matching ordinal or−1 when absent. The inverse depends on corresponding restored list order; “session only” in the new header is not an additional file-format restriction. [coaster8.c](../../LEGOLAND/coaster8.c), [schoolcar8.c](../../LEGOLAND/schoolcar8.c)

The archive loads parallel `.lms` mesh, `.lfm` template and `.ltx` image data. LMS fixup rebases pointer fields +0c,+10,+14,+18,+20,+28 within the loaded block. The seated-rider consumer establishes three 16-byte-record lists: pointer/count pairs +18/+1c and +20/+24 identify colour substitutions, +28/+2c identifies part substitutions. Each record starts with a signed-short index into the copied template's 12-byte records; a matched palette/texture index in the first dword is replaced. Original render consumers now identify all array fields and triangle records: positions/normals, flat-colored/smooth-colored/textured faces, optional bounding box, LFM palette/texture indices and UV bytes. Complete offsets, asset manifests and validators are in [transport-data.md](transport-data.md#lms-lfm-ltx-and-palette-assets). Model-list loading reads both `<name>.obj` and `<name>.txt`, fails on a short whole-file read, and releases the first image if the second fails. Indexed model names append a four-digit decimal index to the base name. [schoolcar7.c](../../LEGOLAND/schoolcar7.c), [coaster6.c](../../LEGOLAND/coaster6.c), [schoolcar4.c](../../LEGOLAND/schoolcar4.c), [schoolcar5.c](../../LEGOLAND/schoolcar5.c), [schoolcar8.c](../../LEGOLAND/schoolcar8.c)
...
`ModelRecord_GetName` finds the indexed record, returns0 without changing output on failure, or copies its complete CRLF-delimited line, appends NUL and returns1. `FindCoasterColour` reads the palette count once and returns the first index whose low24 RGB bits match, ignoring the high byte; absent returns−1, not a null pointer. `Vec3Cross` writes `(ay*bz−az*by, az*bx−ax*bz, ax*by−ay*bx)` in x/y/z order. `Raster_SaveState` returns0 if surface acquisition fails; otherwise it sets raster bits to the returned pointer, pixel pitch to byte pitch>>1 and returns1. Its partner0x423790 is a no-op, so the name does not promise restored globals. [coaster8.c](../../LEGOLAND/coaster8.c), [coastertiny.c](../../LEGOLAND/coastertiny.c)

`ModelImage_IsEOL` reads a little-endian word equal to0x0a0d. `ModelImage_FindRecord` returns the data pointer immediately for index0, regardless of length; for other indices it scans while p<end−1, counts CRLF pairs and returns just after the matching pair. A final delimiter can therefore return end itself. `ModelRecord_CopyToken` copies the **whole line**, including spaces, until CRLF; it appends no NUL and has no length guard. The outer GetName adds NUL. `CoasterModel_FindMeshIndex` and `CoasterModel_FindPartIndex` use the OBJ and TXT pointer/length views at0x4dd860/0x4dd758 respectively and return the case-insensitive matching line index, or−1. An absent delimiter or a returned end pointer can expose an out-of-bounds read; the historical “token” name must not introduce whitespace splitting. [coaster9.c](../../LEGOLAND/coaster9.c), [coaster8.c](../../LEGOLAND/coaster8.c), [transport-data.md](transport-data.md#coaster9-helper-reconciliation)
...
Eight support vertices copy from `0x004b61e0` to `0x00614858`, scaling x by 2.5 and y by 8.2, preserving z bits. Support rendering uses model `0x004b6300`, texture `0x004b62f0` and mode 1; car rendering first sets clip depth and uses mode 0 with the three-float position followed immediately by its 3×3 basis. Shade entry i is allocation-base+128*i and is stored in ramp pointer i before filling. Cursor evaluation uses mode table `0x004b6408` = `[0,2,1]`, current parameter and4.8f offset. The static support/shadow vertices, normals, topology and material inputs are fully recovered in [transport-data.md](transport-data.md#static-support-and-shadow-models). The line and arc table up-vector helpers both return `(0,0,1)`; 0x421cc0 retains its historical `CubicUpVector` name despite installation in the arc table. [coastertiny.c](../../LEGOLAND/coastertiny.c)
...
The new helpers make slot roles explicit: `RoutePhys_SetState` is the solver+00 writer; `RouteSeat_InitPosition` installs four inline seat methods; the query is used by free-seat search and the draw method receives the seat and a frame. Raster acquisition, model-name/palette lookup, route allocation, joint algebra and rider-ordinal lookup are ordinary helpers, not ObjDef registrations. [coaster8.c](../../LEGOLAND/coaster8.c), [transport-data.md](transport-data.md#complete-track-piece-and-route-node-storage)

`PhysObj_ReadState` calls vector-copy+14 as `(out,state+38)`; `PhysObj_WriteState` reverses those arguments. These generic copy adapters are distinct from the route's train-positioning state writer. `RouteSeat_IsOccupied` tests the car pointer. `RouteSeat_ReleaseCar` names0x4273f0 unambiguously; `RouteSeat_Update` takes the second transform argument despite the older one-argument typedef. It adds basis×local-seat to the saved translation, invokes car+20 with that transform, then restores all three translation components. `TrackCurve_EvaluateOffset` similarly evaluates position and subtracts up-vector×offset componentwise. Raw-int float arguments preserve their32-bit cdecl bits. [coaster9.c](../../LEGOLAND/coaster9.c), [coaster8.c](../../LEGOLAND/coaster8.c), [codex-e.md](../lanes/codex-e.md)
...
`SoundSource` is16 bytes `{kind+00,obj+04,x+08,y+0c}`. Both power-station add callbacks construct kind2 map-square sources and leave obj+04 unwritten. [lfmisc2.c](../../LEGOLAND/lfmisc2.c)
...
`SmallPowerStation_Add` and `CrystalPowerStation_Add` first call `AddBasicObject`, then play their sample with arguments(1,1) at the supplied square. Small uses the second FX record's resolved sample, crystal the first. Both ignore the playback return. [lfmisc2.c](../../LEGOLAND/lfmisc2.c)
...
The power-station FX list begins0x4b8750 and contains two12-byte records, with resolved samples at record+08: crystal0x4b8758, small0x4b8764. These are runtime-resolved pointers, not on-disk audio payloads. [lfmisc2.c](../../LEGOLAND/lfmisc2.c)
...
The power-station source's obj+04 contains uninitialized stack contents, because neither add body writes it before playback. This is preserved original behavior; the kind2 coordinates are initialized. [lfmisc2.c](../../LEGOLAND/lfmisc2.c)
...
`SmallPowerStation_Add`0x452ad0 and `CrystalPowerStation_Add`0x452b20 occupy each named class's ObjDef+98. They share `PowerStation_AC`0x452ab0 and sound initialization; their differing sample selection is the only add-body distinction. [lfmisc2.c](../../LEGOLAND/lfmisc2.c)

## Remaining implementation boundaries

All 35 assigned source contracts are documented. The previously external water/boat tables, flume footprints/overlay thresholds/image lists, coaster descriptor masks/source geometry/support models/seat data and model-format boundaries are recovered from the original executable and assets, with reproducible hashes and parsers in [transport-data.md](transport-data.md). Full decoration allocations, track-piece geometry storage and route-node cursor/seat/clip storage are also resolved there.

Exact `LFPath_Point(1)` reads beyond2-point paths;4-point indexing depends on x87 precision. Callers normalize only parameters greater than1. [transport-data.md](transport-data.md#flume-endpoint-arithmetic) The runtime contract preserves those comparisons; whether normal play reaches equality requires execution evidence and is not assumed away. Original unchecked invalid-input behaviour and source WIP matching residuals remain explicitly documented. The extraction checks read the original executable/assets and validate schemas; this documentation pass ran no compiler or gameplay tests. [logflume6.c](../../LEGOLAND/logflume6.c), [logflume7.c](../../LEGOLAND/logflume7.c), [transport-audit.md](transport-audit.md)
