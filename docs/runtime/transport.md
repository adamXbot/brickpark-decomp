...
Cross-checks use [anim2.c](../../LEGOLAND/anim2.c), [posstep.c](../../LEGOLAND/posstep.c), [goldrush.c](../../LEGOLAND/goldrush.c), [rides.c](../../LEGOLAND/rides.c), [ridemisc2.c](../../LEGOLAND/ridemisc2.c), and the cited lane notes. They are supplementary sources, not additional rows in this page's assigned-file inventory. The source-by-source completeness check is recorded in [transport-audit.md](transport-audit.md).
...
The station-departure hook switches to the station derivative while free-stepping. Once the station endpoint is reached it calls the complete-track start helper, stores the exit game time at owner +e0 and reports the whole supplied interval consumed. Geometry cursors first follow render-object +50; at its end they move through the track node's tail joint and recompute world origin/geometry. [schoolcar8.c](../../LEGOLAND/schoolcar8.c)

Math helpers multiply 4×4 matrices as `out=a*b`, create identity matrices, and form a transform by expanding the 3×3 basis and placing translation in entries 3/7/11. The in-place 2×2 inverse swaps/scales diagonal entries and negates/scales off-diagonals using reciprocal determinant, with no singular guard. Three-vector normalization scales by the reciprocal-square-root hook applied to its squared length, with no zero-length special case. Joint construction clears the destination slot, adds signed-short map offsets to the node square and installs the direction. Span clip forwarding converts its left/top/right/bottom arguments to the stored top/left/bottom/right layout. [coastermath.c](../../LEGOLAND/coastermath.c)
...
| 7 | `TrackCurve_NormalAt` 0x4220e0 | `Sub_421a90` 0x421a90 | `Sub_421cc0` 0x421cc0 |
...
`CarouselRec` is 0x24 bytes: +00 next, +04 square, +06 boarded, +07 aboard, +08 signed frame, +0c flags, +10 revolution byte, +14 half-rate counter, +18 expected visitors, +1c idle timer, +20 four seat bytes. Rider seat +36 is one-based; pose +35=1 selects riding placement. [bswater.c](../../LEGOLAND/bswater.c)
