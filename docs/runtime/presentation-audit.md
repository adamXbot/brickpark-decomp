...
Stage 1: inventory the 55 assigned sources, their headers, behavior/bug comments and WIP boundaries; compare them with the presentation page.
...
| [bigrender.c](../../LEGOLAND/bigrender.c) | 1,4,5,7 | Scaled versus fatal tiled path; Z stream; cursor footprints, shapes and arrows | 223 | 1 | `3e1710cf4654` |
...
| [fpui4.c](../../LEGOLAND/fpui4.c) | 2,3,5,7 | Flash overlay; pickup duplicates; list leak; horizontal-scroll asymmetry | 139 | 1 | `c65788bc76b0` |
| [fpui5.c](../../LEGOLAND/fpui5.c) | 2,3,7 | 133-price sentinel; work-order cancellation; destructive narration split; NEW shift | 65 | 1 | `3ed0e474e711` |
...
| [popup.c](../../LEGOLAND/popup.c) | 3,4,7 | Details layout; work-order reconciliation; blending/pitch/indeterminate return | 167 | 1 | `de9edfc75c5b` |
...
| [render4.c](../../LEGOLAND/render4.c) | 5,7 | Ground diamond traversal; bridge banks; second-cell wrong coordinates | 59 | 1 | `4555f823ed87` |
...
| [renderview.c](../../LEGOLAND/renderview.c) | 1,5,7 | Five phases; exact gather/slicing; overview switch table; original leaks | 209 | 2 | `f1e1e7a26d61` |
...
| [screens2.c](../../LEGOLAND/screens2.c) | 1,3,7 | Title/profile/options geometry; name editors; confirmation popup | 223 | 1 | `4ba5ccb4dfb0` |
...
`misc3.c` is additional cross-subsystem evidence, not a 56th assigned file: popup size/mock preview, work-order overlay and LLIDB picker were audited here; its ownership remains in the core inventory. The work-order layout and list insertion reconciliation also cite workorder.c/workorder2.c/workorder3.c. [misc3.c](../../LEGOLAND/misc3.c), [workorder.c](../../LEGOLAND/workorder.c), [workorder2.c](../../LEGOLAND/workorder2.c), [workorder3.c](../../LEGOLAND/workorder3.c)

## WIP boundary ledger

WIP status is the source's executable-matching status, not an automatic documentation omission. The recovered behavior of all twelve marked functions is described. The retained distinctions below prevent a reader treating a scheduling residual as an unknown algorithm or a known reconstruction error as shipped behavior. [presentation](presentation.md)
...
| PaintTileLayer `0x4608c0` | Callee-saved register permutation; block layout matches | Traversal/overlay rules and wrong second-cell position documented. [render4.c](../../LEGOLAND/render4.c) |
| RenderView `0x45b180` | Two acknowledged reconstruction errors: x/y loop limits belong after quadrant switch; slice-count reset belongs inside nonnull-object branch | Original-note placement is explicitly documented; current C difference is not presented as an original bug. [renderview.c](../../LEGOLAND/renderview.c) |
| RenderFullMap `0x4567a0` | Shared-tail/else-arm layout mismatch | Full scaling, terrain, object, coaster, marker and lifecycle contracts documented. [renderview.c](../../LEGOLAND/renderview.c) |
...
| --- | --- | --- |
| Assigned-source coverage | Compare the 55 ledger filenames and the introductory presentation-table source links with the presentation inventory; require set equality and 55 unique rows | Passed, 55/55 in each |
| Source fingerprints | Compute SHA-256 for each linked source and compare its first 12 hexadecimal characters with the ledger | Passed, 55/55 |
| Comment/WIP inventory | Compare each Blocks count with `source.count('/*')`; compare each WIP count with line-anchored `// WIP-FUNCTION` markers | Passed, 6,271 block comments and 12 WIP markers |
| Relative links | Resolve every Markdown target in both pages against its containing directory, ignoring only URL targets and fragments | Passed, 588 local links and no missing targets |
...
| Coverage vocabulary | Require Partial for the five groups with material source gaps; distinguish WIP executable-matching residuals from unknown runtime behavior | Passed, 17 Partial / 38 Documented assigned sources |
