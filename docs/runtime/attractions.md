...
| Spider `0x30` | `0x004cbf58` | `+00`; `+2c` | `+02/+03 u8 seated/riders`, `+04 i8 frame`, `+14 u8 joined`, `+18 i32 timer`, one-based seat addressing `+1b+seat`, physical array `+1c`. [mechrides.c](../../LEGOLAND/mechrides.c) |
| Barrels `0x34` | `0x0062fe08` | `+04`; `+00` | `+06/+07 u8 seated/riders`, `+08 i8 layer3 frame`, `+18 u8 joined`, `+1c i32 timer`, `+20 i8 layer2 frame`, one-based seat addressing `+20+seat`, physical array `+21`. [mechrides.c](../../LEGOLAND/mechrides.c) |
| Plane/Zoomer `0x24` | `0x0062fe9c` | `+00`; `+20` | `+02/+03 u8 seated/riders`, `+04/+05 i8 layer1/2 frame`, `+14 u8 joined`, `+18 i32 timer`; one-based seat addressing `+1b+seat`, physical array `+1c`. [mechrides.c](../../LEGOLAND/mechrides.c) |
...
The apparent Spider/Plane/Barrels seat overlap is a **one-based addressing bias** in the recovered valid-seat contract: allocator notes explicitly place the first occupied byte at `+1c` for Spider/Plane and `+21` for Barrels, while release accesses `+1b+seat` or `+20+seat`. Equating those addresses gives seat ID 1 for physical slot 0. Spider also has a leading zero in its path-limit table, and the seat IDs become two-digit BNV path names. Thus valid seats do not overlap the timer/frame; only an invalid seat ID 0 would hit the preceding byte. This reconciles the early “seat 0 overlaps” prose using the later allocator-disassembly notes and consumers; the external allocator bodies themselves are still absent. [mechrides.c](../../LEGOLAND/mechrides.c), [ridecb3.c](../../LEGOLAND/ridecb3.c)
...
| Record | Layout |
| --- | --- |
| Carousel `0x24` | Head `0x006160c4`; next `+00`, square `+04`, boarded/aboard bytes `+06/+07`, signed frame `+08`, flags `i32 +0c` (`1 running,0x4000 boarding closed to newcomers`), revolutions `u8 +10`, half-tick counter `i32 +14`, visitor byte `+18`, timer `i32 +1c`, four occupancy bytes `+20..+23`. Seat IDs in blokes are **one-based**, while storage is zero-based: release `seat[b->seat-1]`; the older `+1f` reading is a biased displacement, not the array start. [ridecb3.c](../../LEGOLAND/ridecb3.c), [bswater.c](../../LEGOLAND/bswater.c) |
...
The boating/jungle station rider queues both have five slots. Action0 appends at4 or rejects a full/occupied tail; existing queuers advance when the next slot is free, and reaching0 selects action1. Boating action1 requires a route and take≥six times water-square count, then attempts launch; success sits the rider, clears queue0, decrements its count and plays the launch sound pitched+10. Action2 holds. Action3 restores walking, clears riding, snaps to four tiles west/two south of the queue exit square and targets fixed-point offset `(-0xc0,+0x240)`;4 targets `(-0xc0,+0x80)`;5 targets centre and fades rider sounds over90;6 leaves. [ridecb5.c](../../LEGOLAND/ridecb5.c)
