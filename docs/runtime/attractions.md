...
| Gold Rush, fort, castle level 1, temple | [goldrush.c](../../LEGOLAND/goldrush.c), [goldrush2.c](../../LEGOLAND/goldrush2.c), [goldrush3.c](../../LEGOLAND/goldrush3.c), [goldrush4.c](../../LEGOLAND/goldrush4.c) | Documented recovered scripts/tables; pan-drift prose conflict flagged |
| Joust and temple slide | [joust.c](../../LEGOLAND/joust.c), [joust2.c](../../LEGOLAND/joust2.c) | Partial: rider/cycle mechanics recovered; external BNV data required |
...
| Water Works and garden | [waterworks.c](../../LEGOLAND/waterworks.c) | Documented; contradictory garden fallback prose flagged |
| Castle and track-class adapter | [castleobj.c](../../LEGOLAND/castleobj.c) | Partial: adapter/layout documented; coaster geometry belongs with transport |
| Entrance, mechanics hut and transport callback frontier | [ridecb2.c](../../LEGOLAND/ridecb2.c), [ridecb5.c](../../LEGOLAND/ridecb5.c), [ridecb6.c](../../LEGOLAND/ridecb6.c), [ridecb7.c](../../LEGOLAND/ridecb7.c), [ridecb8.c](../../LEGOLAND/ridecb8.c), [ridecb9.c](../../LEGOLAND/ridecb9.c) | Partial: recovered callback contracts and faults; transport page supplies full routes/vehicles |
...
| Bloke fields used by rides | `+04 person`; `+0e u16` low-level state (`0` idle, `7` walking); `+24/+28 i32` target; `+34 i8` walk-path direction; `+35` ride pose; `+36 u8` seat/pan; `+37` occlusion band; `+38 i16` path/frame cursor; `+3c/+3e i16` in-ride offsets; `+40/+42 i16` fort substate/resume; `+4a/+4c i16` animation part/stop; `+50 i32` animation ID; `+54` BNV path; `+58 i32` timer; `+60 u8` ride action; `+62 u16` flags; `+68/+6c i32` world; `+72/+73 u8` facing/new facing; `+98` movement scratch. Signedness of `+38/+4a/+4c` differs between local declarations: the tower animation consumer sign-extends them. [goldrush.c](../../LEGOLAND/goldrush.c), [mechrides.c](../../LEGOLAND/mechrides.c), [ridemisc2.c](../../LEGOLAND/ridemisc2.c) |
...
| --- | --- | --- | --- |
| Safari `0x28` | `0x004cbf0c` | `+00`; `+10` | `+04 i32 seated`, `+08 i32 riders`, `+0c i32 frame`, `+20 i32 joined`, `+24 i32 timer`. [mechrides.c](../../LEGOLAND/mechrides.c) |
| Spider `0x30` | `0x004cbf58` | `+00`; `+2c` | `+02/+03 u8 seated/riders`, `+04 i8 frame`, `+14 u8 joined`, `+18 i32 timer`, seat addressing `+1b+seat`; allocator notes use base `+1c`. [mechrides.c](../../LEGOLAND/mechrides.c) |
| Barrels `0x34` | `0x0062fe08` | `+04`; `+00` | `+06/+07 u8 seated/riders`, `+08 i8 layer3 frame`, `+18 u8 joined`, `+1c i32 timer`, `+20 i8 layer2 frame`, seat addressing `+20+seat`; allocator base `+21`. [mechrides.c](../../LEGOLAND/mechrides.c) |
| Plane/Zoomer `0x24` | `0x0062fe9c` | `+00`; `+20` | `+02/+03 u8 seated/riders`, `+04/+05 i8 layer1/2 frame`, `+14 u8 joined`, `+18 i32 timer`; seat addressing `+1b+seat`, allocator base `+1c`. [mechrides.c](../../LEGOLAND/mechrides.c) |
...
| Tower `0xb4` | `0x0062fda8` | `+00`; `+08` | `+02/+03/+04 u8 seated/riders/joined`; four cars `+14..+a4`; eight occupancy bytes `+a4..+ab`; `+ac/+ad i8 layer3/5 frames`; `+b0 i32 timer`. [bswater3.c](../../LEGOLAND/bswater3.c) |
...
| Tower geometry tables | Seat offsets: eight-byte rows at `0x004b77e8`; car facing rows: twenty bytes at `0x004b77a8`; animation refs: eight bytes at `0x004b775c`. Headers identify shapes but do not supply all literal rows. [mechrides.c](../../LEGOLAND/mechrides.c), [ridemisc2.c](../../LEGOLAND/ridemisc2.c) |
...
`mechrides.c` labels seat 0 as overlapping Spider/Plane timer byte `+1b` and Barrels layer frame `+20`, but later allocator notes identify bases `+1c`/`+21`; release code uses the earlier address **plus the seat index**. This is consistent with one-based seat IDs (as established for Carousel), but the allocator bodies are not present in these assigned sources. Preserve the explicit addresses/indexing and treat the alleged real overlap as unresolved, rather than flattening it into an asserted runtime bug. [mechrides.c](../../LEGOLAND/mechrides.c), [ridecb3.c](../../LEGOLAND/ridecb3.c)
...
Temple's walk is a fixed out-and-back sequence relative to the anchor: `(-2,-4)`, `(-2,-6.5)`, `(-2.375,-8)`, `(-2,-9.5)`, `(-2.375,-12)`, then the reverse through `(-2,-9.5)`, `(-2.375,-8)`, `(-2,-6.5)`, `(-2,-4)`, centre `(0.5,0.5)`, then leave. Castle level 1 uses a seven-state walk/wait visit; its random x jitter has the irregular mapping recorded below. All four set/clear the shared “using ride” flag and draw people behind building mattes. [goldrush.c](../../LEGOLAND/goldrush.c)
...
The pan header explicitly describes a **`0x30` southward drift** from `-0x50` kneeling versus `+0x80` standing and says state 9 prevents accumulation across visits. Its coordinate formulas are more precise than that explanation: the kneel target changes from pan y `+0x80` to `-0x50` (a `-0xd0` target displacement), standing directly shifts world y `+0x80` and subsequently walks to a fixed target. The claimed net drift is therefore not proven by these operations alone. Preserve both exact operations, and treat the `0x30` description as an unresolved interpretation rather than adding a further drift operation. [goldrush3.c](../../LEGOLAND/goldrush3.c)

Castle level 1's `rand()&3` jitter maps `0→-128`, `1→0`, `2→128`, `3→3`, giving a three-unit nudge on one roll in four. Its loader reads `def->layers` outside the null guard. Gold paving only changes path graphics, while removal performs full path removal with no refund. Gold's record unlink has the shared null-head fault. [goldrush.c](../../LEGOLAND/goldrush.c), [ridemisc3.c](../../LEGOLAND/ridemisc3.c)
...
Temple Slide allocates one of four lanes, routes the customer through the entrance according to lane, mounts the corresponding BNV path, changes from walking to sliding using a lane-specific frame threshold, then releases the lane and walks out. Admission is recomputed from occupied lanes on release. Its BNV path origin uses the same ride projection contract above. The per-lane walk/end frame values remain external data, so a complete timed slide requires those assets/tables. [joust.c](../../LEGOLAND/joust.c)

### Tables and constants

Joust's BNV name is `manBox%02d` with horse+1; the cycle is 64 frames, horses half a cycle apart, watch/ride waits 150 ticks, spectator seats six. Temple slide path-name table `0x004b4f08` contains `manbox01..04`; four signed walk thresholds start at `0x004b4f18`, end thresholds at `0x004b4f1c`, but their values are not decoded in the assigned header. Joust FX sample is at entry offset `+08`, correcting `joust.c`'s old `sample@+04` declaration. [joust.c](../../LEGOLAND/joust.c), [joust2.c](../../LEGOLAND/joust2.c)
...
| Record | Layout |
| --- | --- |
| Carousel | Head `0x006160c4`; next `+00`, square `+04`, boarded/aboard bytes `+06/+07`, signed frame `+08`, flags `i32 +0c` (`1 running,0x4000 accepting`), revolutions `u8 +10`, half-tick counter `i32 +14`, visitor byte `+18`, timer `i32 +1c`, occupancy array `+20`. Seat IDs in blokes are **one-based**, while storage is zero-based: release `seat[b->seat-1]`; the older `+1f` reading is a biased displacement, not the array start. [ridecb3.c](../../LEGOLAND/ridecb3.c) |
...
Carousel actions `0` join/180 timer/approach, `1` select seat and mount `CarouselOn` BNV, `2` play until complete then action 5, `5` sit/count boarded/start at capacity, `7` stand and mount `CarouselOff`, `8` play then action 13 facing 3, `13` free seat and return to map, `14` leave/reopen. Actions 3,4,6,9..12 hold. Starting copies boarded to aboard, clears boarded, sets running, clears accepting and starts sound. After rider ticks, all carousel machines tick; missing placement records skip that phase. `Carousel_StopRide` resets half-tick/platform counters, visitors and boarded count, chooses `rand()%2+3` revolutions, clears flags `0x4001` and fades the square sound after the machine has released its riders. [ridecb3.c](../../LEGOLAND/ridecb3.c), [ridemisc3.c](../../LEGOLAND/ridemisc3.c)

Balloonz has six gondolas and two 24-frame halves. Platform car is `wheel/8 + half*3`; the BNV frame is `zframe + half*24`. Actions `0..6` approach through fixed queue points, `7` waits for a docked gondola and claims it, `8` approaches its door, `9` mounts, `10` rides, `11` marks finished, `12` waits for its car to return while unloading, `13` clears platform, `14` walks away and frees car, `15` leaves. Car states are `0 free,1 claimed,2 boarding,3 finished`. The wheel advances only while occupied and not held by boarding/unloading or the one-in-three pickup choice. zframe intentionally lags the wheel one tick. [ridecb3.c](../../LEGOLAND/ridecb3.c)
