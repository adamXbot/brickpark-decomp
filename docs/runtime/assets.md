...
| --- | --- | --- |
| `.res` | u32 directory offset; packed member data from+4; tail tree nodes `{child offset,next sibling offset,folder flag,size,data offset,NUL name}`. Tree offsets are relative to directory base;`ffffffff` terminates links. [Full-path proof](core-data.md#resource-directory-paths) | [FORMATS.md](../FORMATS.md), [RE_CONTEXT.md](../RE_CONTEXT.md) |
...
| `.odf` | Builds`0xd0` ObjDef, resolves sprite/icon/build sprite/child, retains English/record0 localization, then standard callbacks, library-or-custom branch and three-name finalizer; see the callback index for the DLL-failure fallback | [llidb_odf.c](../../LEGOLAND/llidb_odf.c) |
...
`RE_CONTEXT.md` is an early snapshot: it treats COMP substreams as unknown and reports different discovery counts. The later decoded COMP format in `FORMATS.md` is used below. `FORMATS.md` itself contains conflicting abbreviated TSM/ILF descriptions: use the actual `llidb_load.c` read sequence above, not its “per-entry mappingName/tilesetName” TSM shorthand or ILF list without offsets. The recovery pass now checks original resource bytes using full directory paths, including all15 initialized morph animations, four LOC variants and the ride-specific ILF/geometry data. [Character data](core-data.md#character-asset-manifest), [transport data](transport-data.md) [RE_CONTEXT.md](../RE_CONTEXT.md), [FORMATS.md](../FORMATS.md), [llidb_load.c](../../LEGOLAND/llidb_load.c)
...
Ground/path sprites are32×16; tile width is twice height. Coaster design LTX is width/height dwords plus width×height bytes (shipped examples32×32); LPT has count145 and BGR0 entries. The complete `.lms/.lfm` record schema and all10 shipped model/material pairs are in [transport data](transport-data.md). The old LoadBaseMap note about decoding map flags again as base tiles describes a **fixed reconstruction error**, not an original bug to preserve. Asset callbacks resolve through LLIDB and class installation. [FORMATS.md](../FORMATS.md), [loadmap.c](../../LEGOLAND/loadmap.c)
...
| Person3D | `0x94`; kind`+8`, Bloke`+c`, screen`+1c/+20`, scale`+10..18`, depth weights`+34/+38`, Euler rotation`+40..48`, frame`+4c`, private face/part array`+50`, depth`+54`, orientation`+58..78`, appearance selectors`+7c/+80`, sex`+84`, animation`+88`, leg/arm colours`+8c/+90` | [person3d.c](../../LEGOLAND/person3d.c), [blokeanim.c](../../LEGOLAND/blokeanim.c) |
| `.loc` | texture count`+0`, runtime texture-ID base`+4`, patch count`+8`,32-byte stem`+c..2b`, relative texture-table/patch pointers`+2c/+30` relocated after load | [data3.c](../../LEGOLAND/data3.c), [lanes/fable-c-data3save.md](../lanes/fable-c-data3save.md) |
...
| Position table at loader`0x43f660` | records-per-stream then stream-count, followed by streams×records×48-byte `{three raw dwords,matrix[9]}` records;40-byte descriptor. Scalar interpretation is consumer-specific; Copters uses+4 as float y and+8 is DWORD4 | [loaders.c](../../LEGOLAND/loaders.c) |
...
| BINV/BNV | magic`0x0101`, frame count u16+2, frame-list offset20; names/vertices/links are file-relative pointers relocated on load. Name nodes next4, vertices8, name c, orientation10; vertex is20 bytes with short projected x/y, float depth+4, and three authored scalars at8/c/10; GetZSkew consumes c/10 | [rin.c](../../LEGOLAND/rin.c), [bnvpath.c](../../LEGOLAND/bnvpath.c) |
...
`FORMATS.md` calls loader`0x43f660` a `.3d` loader and interprets asset bytes as rigid transforms; the current source names that function `LoadPos`, while `LoadAnim3D` at`0x43fa80` reads morph meshes. These are distinct parsers. Prefer the appropriate current loader contract; the original member checks now resolve the count order and stream variants: morph meshes and rigid position streams use their respective schemas. [Verified character/position schemas](core-data.md), [Copters and Earth streams](attractions-data.md) [FORMATS.md](../FORMATS.md), [loaders.c](../../LEGOLAND/loaders.c), [person3d.c](../../LEGOLAND/person3d.c)
...
Viewport for sound is config`+10/+12`, not outer screen dimensions`+0/+2`. Unknown SoundSource kinds leave source position or matching flag uninitialized. Sample allocation initializes only observed fields and has no failure guard; narration retains unchecked allocations/API results and unbounded path building. SFX media and the native DirectMusic composition implementation are external inputs/services; no browser audio scheduling or replacement is implemented here. [sysmisc2.c](../../LEGOLAND/sysmisc2.c), [audio3.c](../../LEGOLAND/audio3.c), [sysmisc.c](../../LEGOLAND/sysmisc.c), [sysstubs.c](../../LEGOLAND/sysstubs.c), [audio4.c](../../LEGOLAND/audio4.c), [music.c](../../LEGOLAND/music.c)
...
The build FX table at`0x004b9228` has23 twelve-byte records `{name,pad,sample}`. Class flag`0x40000` selects effect1; otherwise flag`0x80000` selects effect0 when `0x200000` is also set, or `rand()%5+3` (indices3…7) without it; other classes select0. First-name table sizes are90 and83, surname table107; their complete original strings are in [core data](core-data.md#male-first-names). [mapinit.c](../../LEGOLAND/mapinit.c), [loaders.c](../../LEGOLAND/loaders.c), [blokelist.c](../../LEGOLAND/blokelist.c)

`LoadPos` establishes three raw dwords followed by nine matrix floats. Copters resolves the ambiguity: it reads+4 as float y, but+8 is DWORD4 in the shipped stream, so the format is not universally float XYZ. [Consumer and asset evidence](core-data.md#position-streams) [loaders.c](../../LEGOLAND/loaders.c)

The recovery pass resolves the former schema gaps: BNV c/10 drive GetZSkew; LOC+4 is a texture-ID base and its32-byte stem/patch counts are demonstrated by real assets; person scale, rotation and mask roles follow their consumers. All initialized animations, boat artwork/seat tables and generic position variants now have source/data evidence. Unused reserved bytes are preserved as such, not assigned invented transforms. [Core recovery](core-data.md), [transport recovery](transport-data.md) Completed leaf and audio-wrapper contracts do not become partial solely because they call native services; historical matching status is tracked separately. [bnvpath.c](../../LEGOLAND/bnvpath.c), [data3.c](../../LEGOLAND/data3.c), [person3d.c](../../LEGOLAND/person3d.c), [blokeanim.c](../../LEGOLAND/blokeanim.c), [anim2.c](../../LEGOLAND/anim2.c)

## Current-main texture and narration helpers

`RegisterTextureImage` allocates a0x2c record, builds it and writes the unchecked texture slot only when allocation succeeds. `UnInitRasterTables` frees each live slot’s blocks`+8/+c`, frees the record and clears that slot across all256 entries. `LoadTextureImage` returns0 on decode failure, destroys a decoded image on conversion failure, and returns the image otherwise. `MakeSpriteDrawable` returns0 only when flag`0x20` requires a recreation that fails; otherwise it calls MakeSprite, ignores that result and returns1. [savemisc2.c](../../LEGOLAND/savemisc2.c)

`UnInit3DPrintList` actually frees the shading-ramp cache’s ramp and node pairs; the head is not cleared. `UnInitModelTextures` frees the four outfit tables without nulling their pointers and returns0. `UnregisterDetailImage` clears a matching slot, decrements the live count and frees/nulls the table when the count reaches0; it returns1 for a found entry,0 otherwise. TSM teardown walks until the kind word−1, unloads each referenced element and frees the table; it does not clear the owning element’s data pointer or return a defined value. [savemisc2.c](../../LEGOLAND/savemisc2.c)

`VolumeFromDistSq(d2)` computes master attenuation−d2/60; results below−3000 or above0 become−10000. `ResetNarrationStreamState` clears the0x50-byte queue and nine cursor/status scalars. `StopNarrationPlayback` returns0 in states0/1; otherwise it stops the DirectSound buffer, resets stream state, rewinds and sets state1, returning1. [savemisc2.c](../../LEGOLAND/savemisc2.c)

## Interactive music worker

### Data structures and initialization

The worker’s three segment banks are first[5] at`0x799230`, second[5] at`0x799244` and transitions[25] at`0x799258`, indexed source×5+destination. Parallel data-pointer/length banks stage30 files: two main segments per world plus20 directed transitions; diagonal transition slots are unused. World order is theme/Legoland, Egypt, Inca, medieval, west. The mailbox is command/argument at`0x79a6a4/a8`, state`0x4bf778`, active world`0x79a6ac`; manual-reset command/notification events are`0x79a6a0/9c`. Notification kind GUID and option precede field1/field2 at`+4c/+50`. [musicthread.c](../../LEGOLAND/musicthread.c)

The thread exits with disabled=1 if its engine instance is absent. Otherwise it initializes COM, composer and performance, creates a default port with24voices, one channel group, two audio channels and22050Hz settings (`dwValidParams=0`), allocates at least18 bytes for the port format, creates/attaches a DirectSound buffer and activates/assigns the port. It creates a loader and two events, searches/caches `imusic`, and scans segment/style files. A successful segment scan gates loading all30 segment blobs; a successful style scan additionally enumerates styles. Each memory object descriptor has flags`0x406`, class Segment, a byte length, data pointer and requested wide name; the blob is freed after GetObject regardless of success. [musicthread.c](../../LEGOLAND/musicthread.c)

### File and name mapping

First/second file stems are `segtheme1/2`, `segegypt1/2`, `seginca1/2`, `segmed1/2`, `segwest1/2`; files have `.sgt` under `imusic`. Requested names equal stems except theme1→`themeintro`,theme2→`theme`. Transition names use source/destination letters l/e/i/m/w plus`tran2`; the file stem matches that name except Egypt→Inca (slot7) reads **ietran2** while requesting **eitran2**. Slot11 correctly reads/requestsietran2. This mismatched request is established; the header’s stronger claim that GetObject necessarily leaves the slot null is not proven by this caller alone. [musicthread.c](../../LEGOLAND/musicthread.c)

### Commands and notification state machine

The worker registers segment and measure/beat notifications. It downloads main segments and outbound transitions only for source worlds1..4; world0 and its four outbound transitions are skipped. Each downloaded segment receives repeat count0. It sets disabled=1 and ready=1, then waits indefinitely on both events, processing a pending command before notifications when both are signalled. [musicthread.c](../../LEGOLAND/musicthread.c)

| Trigger | Behavior and next state |
| --- | --- |
| Command1 | Stop all performance segments; publish state1. |
| Command3 | Set active world=argument, groove0, play its first segment with flag0x2000; publish state3. |
| Command4, same world | Groove0, play that world’s second segment with flag0x2000; publish state4. |
| Command4, different world | Groove1, replace command by5, initialize beat sentinel−1; publish state5. |
| Other nonzero command | No switch action; copy numeric command into state. |
| Segment option2, state3/4 | Queue command4 for active world and signal command event. |
| Segment option4, state6 | Publish state7; other states only log. |
| Segment options0/1/3 | Log start/end/loop respectively. |
| Nonsegment notification, state5 | First stores field1 when beat=−1. At field1=0, a nonzero beat sentinel starts the source→destination transition with flag0x2000 and sets state6; a zero sentinel becomes1, delaying that transition until a subsequent zero-field1 event. |
| Nonsegment notification, state7 | At field1=3,field2=1, set active world=argument, play destination second segment, groove0,state4. |

After handling a command it clears the command mailbox. Notifications are drained until GetNotificationPMsg fails and each retrieved message is freed. The nonsegment path does not separately compare its GUID with measure/beat: this is the literal branch contract. Fields1/2 retain their numeric roles without relying on the header’s inconsistent beat/bar wording. [musicthread.c](../../LEGOLAND/musicthread.c)

### Original failures and lifecycle limits

Port creation and segment-object results are not guarded before later dereferences; failed/missing segment slots can fault in the unconditional download loop. File-open/length/read failures are not converted into a safe empty stream. Short-read logging prints the boolean mismatch rather than bytes read. The0x24-byte DirectSound descriptor leaves its final GUID uninitialized. CreateSoundBuffer failure jumps directly to shutdown, leaking earlier objects and the format block; other initialization failures follow their specific release ladder. Event creation checks only the command event. Successful playback loops forever, with existing teardown terminating the worker rather than a cooperative exit. [musicthread.c](../../LEGOLAND/musicthread.c), [sysmisc3.c](../../LEGOLAND/sysmisc3.c)

The7-instruction WIP residual is documented as placement of imported-function reloads across loop edges; it does not omit music state-machine behavior. No native COM playback or audio parity test was run in Scope J.

The active visitor outfit ordinals are male faces[4,5],chests[0,2,3] and female faces[4],chests[1,2,3], with defaults4/0 and4/1. These are reconstructed from the actual NewProject/altman/altwoman lists and the original loader; [outfit evidence and check](core-data.md#active-outfit-lookup-tables) supply the missing live table values. [data2.c](../../LEGOLAND/data2.c), [savechunks.c](../../LEGOLAND/savechunks.c)
